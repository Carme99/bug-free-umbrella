...
Describe "Get-MailboxHealth" {
    BeforeAll {
        ...
        $mailboxDetail = [pscustomobject]@{
            ...
            IssueWarningQuota    = '1.5 GB (1,610,612,736 bytes)'
        ...
        function Get-EXOMailboxStatistics {
            [CmdletBinding()] param([string]$Identity, [switch]$Archive)
        }
        ...
        Mock Get-Module -ParameterFilter { $Name -eq 'ExchangeOnlineManagement' -and $ListAvailable } {
        ...
        Mock Get-EXOMailbox {
            if ($Identity) { return $mailboxDetail }
            return $mailboxListing
        }
        Mock Get-EXOMailboxStatistics {
            @([pscustomobject]@{
                TotalItemSize = '1 GB (1,073,741,824 bytes)'
                ItemCount     = 1234
    ...
    Context "Help & Metadata" {
        ...
        It "Declares File Name matching the actual filename" {
            $raw | Should -Match 'File Name\s*:\s*Get-MailboxHealth\.ps1'
        }
    ...
    Context "Behavior" {
        It "Runs a healthy audit end-to-end and returns 0" {
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 0
            ($out | Out-String) | Should -Match '\[\+\] Found 1 mailbox\(es\)'
        ...
        It "Is idempotent when already connected" {
            ...
            Main | Should -Be 0
            Should -Invoke Connect-ExchangeOnline -Exactly 0 -Because "the session already exists"
        }

        It "Returns the documented exit code 1 when a mailbox exceeds its send quota" {
            Mock Get-EXOMailboxStatistics {
                @([pscustomobject]@{
                    TotalItemSize = '4 GB (4,294,967,296 bytes)'
                    ItemCount     = 9999
                    LastLogonTime = (Get-Date).AddDays(-1)
                })
            }
            ...
            ($out | Out-String) | Should -Match 'Quota Exceeded: 1'
        }

        It "Tolerates a per-mailbox statistics failure and keeps going" {
            Mock Get-EXOMailboxStatistics { throw 'statistics unavailable' }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 0
            ($out | Out-String) | Should -Match '\[-\] Error processing Alice'
        }
        ...
        It "Returns 1 with [-] output when Exchange Online module is missing" {
            Mock Get-Module -ParameterFilter { $Name -eq 'ExchangeOnlineManagement' -and $ListAvailable } { }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 1
            ($out | Out-String) | Should -Match '\[-\]'
            Should -Invoke Get-ConnectionInformation -Exactly 0 -Because "the module guard fires first"
        }
        ...
        It "Exports an HTML report when -ExportHTML is used" {
            ...
            Main | Should -Be 0
            Should -Invoke Out-File -ParameterFilter { $FilePath -like '*MailboxHealth_*.html' } -Exactly 1
        }
    }
}
