...
# Pester 5 suite for scripts/collaboration/microsoft365/exchange-online/Get-UserMailRules.ps1
# Runs fully offline on Linux pwsh: all Exchange Online cmdlets are mocked by name.

Describe "Get-UserMailRules" {

    BeforeAll {
        # Mirrored layout: Tests/collaboration/microsoft365/exchange-online/ -> repo root is four levels up.
        $scriptRelPath = "../../../../scripts/collaboration/microsoft365/exchange-online/Get-UserMailRules.ps1"
        ...
        function Get-InboxRule { param([string]$Mailbox) }
        function Get-MailboxAutoReplyConfiguration { param([string]$Identity) }
    ...
    Context "Help & Metadata" {
        ...
        It "Declares the actual filename and PowerShell 7.0 prerequisite" {
            $rawContent | Should -Match '(?m)^\s*File Name\s*:\s*Get-UserMailRules\.ps1\b'
    ...
    Context "Behavior" {
        ...
        It "Audits inbox rules and returns 0 when connected and no forwarding is configured" {
            $UserEmail = 'john.doe@contoso.com'
            Mock Get-InboxRule {
                @(
                    [pscustomobject]@{
                        Name                  = 'Newsletter filing'
                        Enabled               = $true
                        Priority              = 1
                        From                  = $null
                        SubjectContainsWords  = @('invoice')
                        BodyContainsWords     = $null
                        ForwardTo             = $null
            ...
            ($out | Out-String) | Should -Match '\[\+\] No mailbox-level forwarding configured'
            ($out | Out-String) | Should -Match '\[\+\] Found 1 inbox rule\(s\) \(1 enabled, 0 disabled\)'
            Should -Invoke Get-InboxRule -Exactly 1 -ParameterFilter { $Mailbox -eq 'john.doe@contoso.com' }
            Should -Invoke Get-MailboxAutoReplyConfiguration -Exactly 1
        }

        It "Flags active forwarding rules with [!] warnings when external forwarding is detected" {
            $UserEmail = 'john.doe@contoso.com'
            Mock Get-EXOMailbox {
                [pscustomobject]@{
                    DisplayName                 = 'John Doe'
            ...
            $out = Main *>&1
            @($out | Where-Object { $_ -is [int] })[-1] | Should -Be 0
            ($out | Out-String) | Should -Match '\[!\] FORWARDING DETECTED - External Address'
            ($out | Out-String) `
            | Should -Match '\[!\] WARNING: Found 1 active rule\(s\) with forwarding or deletion actions'
        }
        ...
        It "Returns 1 with [-] prefixed output when not connected to Exchange Online" {
            ...
            Should -Invoke Get-InboxRule -Times 0 -Exactly -Because 'no work should happen while disconnected'
        }
