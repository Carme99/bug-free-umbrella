...
Describe "Umbrella CLI" -Tag 'CLI' {
    ...
    Context "Discovery — Get-BUScript" {
        ...
        It "-Category cloud returns exactly the catalog cloud-domain entries" {
            # Drift-tolerant: the catalog is regenerated by tools/Build-Catalog.ps1, so
            # hardcoded counts rot whenever scripts are added or stray files are excluded.
            $all = Get-BUScript
            $hits = Get-BUScript -Category 'cloud'
            @($hits).Count | Should -BeGreaterThan 0 -Because "cloud category should match at least one entry regardless of catalog drift"
            @($hits | Where-Object { $_.category -notmatch '(?i)cloud' }).Count | Should -Be 0 -Because "every hit should be in the cloud domain"
            @($hits).Count | Should -Be @($all | Where-Object { $_.category -match '(?i)cloud' }).Count -Because "-Category filtering should return every catalog entry whose category contains cloud"
        }
