...
Describe "Invoke-RemediationCheckSharedFolders" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/network/Invoke-RemediationCheckSharedFolders.ps1'
    ...
    Context "Syntax & Static" {
        ...
        It "Gates the destructive Remove-SmbShare behind SupportsShouldProcess" {
            $raw = Get-Content -Path $scriptPath -Raw
    ...
    Context "Behavior" {
        ...
        It "Removes only unauthorized shares and returns 0" {
            Mock Get-SmbShare {
                @(
                    [pscustomobject]@{ Name = 'C$'; Path = 'C:\' },
                    [pscustomobject]@{ Name = 'HR-Data'; Path = 'D:\HR-Data' }
                )
            }
            $out = Main *>&1
        ...
        It "Is idempotent: approved-only share list removes nothing and returns 0" {
            ...
            $out = Main *>&1
            $out | Where-Object { $_ -is [int] } | Should -Be 0
            Should -Invoke Remove-SmbShare -Times 0 -Exactly -Because "all shares are approved"
        }

        It "Honors -WhatIf: no shares are removed" {
            Mock Get-SmbShare {
                @([pscustomobject]@{ Name = 'Rogue'; Path = 'E:\Rogue' })
            }
            $out = Main -WhatIf *>&1
            $out | Where-Object { $_ -is [int] } | Should -Be 0
            Should -Invoke Remove-SmbShare -Times 0 -Exactly
        }
