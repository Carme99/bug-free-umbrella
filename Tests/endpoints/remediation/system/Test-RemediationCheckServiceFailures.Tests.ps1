...
Describe 'Test-RemediationCheckServiceFailures' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckServiceFailures.ps1'
        ...
        # Parameters are declared on the stubs so Pester -ParameterFilter blocks
        # can bind the named arguments the script passes.
            ...
            Set-Item -Path 'Function:global:Get-WinEvent' -Value {
                ...
                throw 'Get-WinEvent is not available on this platform'
            }
    }

    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Test-RemediationCheckServiceFailures\.ps1'
        ...
        It 'Documents its detect exit-code contract in DESCRIPTION' {
            ...
            $scriptText | Should -Match '(?s)Exit codes?: 0 =.+?service\s+failures found'
        }
    ...
    Context 'Behavior' {
        It 'Returns 0 when no service failures are detected' {
            Mock Get-WinEvent { $null }
            Mock Get-Service { [pscustomobject]@{ Status = 'Running'; StartType = 'Automatic' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] No significant service failures detected'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 1 when service crashes exceed the threshold' {
            Mock Get-WinEvent { $null }
            Mock Get-WinEvent {
                @(
                    [pscustomobject]@{ Id = 7034; Message = "The Spooler service terminated unexpectedly" },
                    [pscustomobject]@{ Id = 7034; Message = "The Spooler service terminated unexpectedly" },
                    [pscustomobject]@{ Id = 7034; Message = "The AudioSrv service terminated unexpectedly" },
                    [pscustomobject]@{ Id = 7034; Message = "TheBITS service terminated unexpectedly" }
                )
            } -ParameterFilter { $FilterHashtable.ID -eq 7034 }
            Mock Get-Service { [pscustomobject]@{ Status = 'Running'; StartType = 'Automatic' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'Detected 4 service crashes'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }

        It 'Returns 1 when a critical service is stopped and not disabled' {
            Mock Get-WinEvent { $null }
            Mock Get-Service { [pscustomobject]@{ Status = 'Running'; StartType = 'Automatic' } }
            Mock Get-Service {
            ...
            $out = Main *>&1
            ($out | Out-String) | Should -Match 'Critical services stopped: wuauserv \(Stopped\)'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }

        It 'Returns 0 for a critical service that is stopped but disabled (intentional)' {
            Mock Get-WinEvent { $null }
