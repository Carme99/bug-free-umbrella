...
Describe 'Invoke-RemediationCheckBatteryHealth' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckBatteryHealth.ps1'
        ...
        # Contain the run: temp reports land in the Pester test drive.
        $env:TEMP = "$TestDrive"
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckBatteryHealth\.ps1'
    ...
    Context 'Behavior' {
        It 'Generates the report and returns 0 when powercfg succeeds' {
            Mock Get-CimInstance { [pscustomobject]@{ Chemistry = 2; Status = 'OK' } }
            Main | Should -Be 0
            Should -Invoke Invoke-PowerCfg -Times 1 -Exactly
        }

        It 'Returns 0 without changes when no battery is present (idempotent)' {
        ...
        It 'Returns 1 and writes [-] prefixed output when powercfg fails' {
            Mock Invoke-PowerCfg { 301 }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[-\]'
