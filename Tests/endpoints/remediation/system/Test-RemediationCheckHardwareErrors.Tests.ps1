...
Describe 'Test-RemediationCheckHardwareErrors' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckHardwareErrors.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Test-RemediationCheckHardwareErrors\.ps1'
        ...
        It 'Documents its detect exit-code contract in DESCRIPTION' {
            ...
            $scriptText | Should -Match '(?s)Exit codes?: 0 =.+?hardware errors found'
        }
    ...
    Context 'Behavior' {
        It 'Returns 0 when no hardware errors are detected' {
        ...
        It 'Returns 1 when WHEA hardware errors are present' {
            Mock Get-WinEvent { $null }
            Mock Get-WinEvent {
                @([pscustomobject]@{ Id = 18; ProviderName = 'Microsoft-Windows-WHEA-Logger'; Message = 'internal hardware error' })
            } -ParameterFilter { $FilterHashtable.ProviderName -eq 'Microsoft-Windows-WHEA-Logger' }
            Mock Get-CimInstance { $null }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
        ...
        It 'Returns 1 when SMART predicts disk failure' {
            Mock Get-WinEvent { $null }
            Mock Get-CimInstance { [pscustomobject]@{ PredictFailure = $true; InstanceName = 'Disk0' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match 'SMART failure prediction|Disk failure predicted'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }
