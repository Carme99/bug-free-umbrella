...
Describe 'Test-RemediationCheckMemoryDiagnostics' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckMemoryDiagnostics.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Test-RemediationCheckMemoryDiagnostics\.ps1'
        ...
        It 'Documents its detect exit-code contract in DESCRIPTION' {
            ...
            $scriptText | Should -Match '(?s)Exit codes?: 0 =.+?memory errors found'
        }
    ...
    Context 'Behavior' {
        It 'Returns 0 when no memory errors are detected' {
        ...
        It 'Returns 1 when memory diagnostic errors are present' {
            Mock Get-WinEvent { $null }
            Mock Get-WinEvent {
                @([pscustomobject]@{ Id = 1; ProviderName = 'Microsoft-Windows-MemoryDiagnostics-Results'; Level = 2 })
            } -ParameterFilter { $FilterHashtable.ProviderName -eq 'Microsoft-Windows-MemoryDiagnostics-Results' }
            Mock Get-CimInstance { $null }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'memory diagnostic errors'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }

        It 'Returns 1 when a memory module reports a non-OK status' {
            Mock Get-WinEvent { $null }
            Mock Get-CimInstance {
                @([pscustomobject]@{ DeviceLocator = 'DIMM0'; Status = 'Degraded' })
            }
            $out = Main *>&1
            ($out | Out-String) | Should -Match 'DIMM0 status: Degraded'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }
