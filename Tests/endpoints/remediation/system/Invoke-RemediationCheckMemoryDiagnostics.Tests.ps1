...
Describe 'Invoke-RemediationCheckMemoryDiagnostics' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckMemoryDiagnostics.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckMemoryDiagnostics\.ps1'
        ...
        It 'Declares SupportsShouldProcess for its boot configuration mutations' {
    ...
    Context 'Behavior' {
        It 'Schedules the diagnostic via both native wrappers and returns 0' {
            Mock Get-BcdBootSequence { @('Windows Boot Manager', '--------------------', 'device              partition=\Device\HarddiskVolume1') }
            Mock Invoke-BcdEdit { 0 }
            Mock Invoke-MdSched { 0 }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] Scheduled Windows Memory Diagnostic'
            Should -Invoke Invoke-BcdEdit -Times 1 -Exactly
            Should -Invoke Invoke-MdSched -Times 1 -Exactly
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 0 without mutation when a diagnostic is already queued (idempotent)' {
            Mock Get-BcdBootSequence { @('identifier              {bootmgr}', 'bootsequence            {memdiag}') }
            Mock Invoke-BcdEdit { 0 }
            Mock Invoke-MdSched { 0 }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] Already scheduled'
            Should -Invoke Invoke-BcdEdit -Times 0 -Exactly
            Should -Invoke Invoke-MdSched -Times 0 -Exactly
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Performs no mutation under -WhatIf and still returns 0' {
            Mock Get-BcdBootSequence { @('Windows Boot Manager') }
            Mock Invoke-BcdEdit { 0 }
            Mock Invoke-MdSched { 0 }
        ...
        It 'Returns 1 and writes [-] prefixed output when MdSched fails' {
            Mock Get-BcdBootSequence { @('Windows Boot Manager') }
            Mock Invoke-BcdEdit { 0 }
            Mock Invoke-MdSched { 16 }
            $out = Main *>&1
