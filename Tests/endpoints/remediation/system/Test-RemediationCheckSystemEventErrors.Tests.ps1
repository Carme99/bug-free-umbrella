...
Describe 'Test-RemediationCheckSystemEventErrors' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckSystemEventErrors.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Test-RemediationCheckSystemEventErrors\.ps1'
    ...
    Context 'Behavior' {
        It 'Returns 0 when no critical events are found (compliant)' {
            Mock Get-WinEvent { $null }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] No critical system errors detected'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 1 when known-critical event IDs fire (issue detected)' {
            Mock Get-WinEvent {
                [pscustomobject]@{
                    ProviderName = 'Disk'
                    Level        = 1
                    ID           = 7
                    Message      = 'Bad block detected.'
        ...
        It 'Returns 1 when provider error counts exceed their threshold' {
            Mock Get-WinEvent {
                param($FilterHashtable)
                if ($FilterHashtable.ProviderName -eq 'volsnap') {
                    @(
                        [pscustomobject]@{ ProviderName = 'volsnap'; Message = 'snap 1'; TimeCreated = (Get-Date) },
                        [pscustomobject]@{ ProviderName = 'volsnap'; Message = 'snap 2'; TimeCreated = (Get-Date) },
                        [pscustomobject]@{ ProviderName = 'volsnap'; Message = 'snap 3'; TimeCreated = (Get-Date) },
                        [pscustomobject]@{ ProviderName = 'volsnap'; Message = 'snap 4'; TimeCreated = (Get-Date) }
                    )
                }
                else {
                    $null
            ...
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\].*Detected Volume snapshot errors'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }
