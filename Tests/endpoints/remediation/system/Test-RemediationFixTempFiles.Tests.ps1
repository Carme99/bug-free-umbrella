...
Describe "Test-RemediationFixTempFiles" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/system/Test-RemediationFixTempFiles.ps1'
        $scriptPath = Join-Path $repoRoot $scriptRelPath

        # Safe: the script's top-level guard skips Main when dot-sourced.
        . $scriptPath

        # $env:TEMP is unset on Linux pwsh; point it at /tmp so the temp-path loop runs.
        $env:TEMP = '/tmp'
    }
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Test-RemediationFixTempFiles\.ps1'
    ...
    Context "Behavior" {
        It "Returns 1 with [!] output when stale temp files exceed the threshold" {
            function Get-CimInstance { }
            Mock Get-CimInstance { @([pscustomobject]@{ Special = $false; LocalPath = '/tmp/testuser' }) }
            Mock Get-ChildItem {
@([pscustomobject]@{ FullName = "$env:TEMP\junk.log"; Length = 2GB; LastWriteTime =
                (Get-Date).AddDays(-10) })
            }
            $out = Main *>&1
        ...
        It "Is idempotent: temp usage below the threshold returns 0" {
            function Get-CimInstance { }
            Mock Get-CimInstance { @([pscustomobject]@{ Special = $false; LocalPath = '/tmp/testuser' }) }
            Mock Get-ChildItem {
@([pscustomobject]@{ FullName = "$env:TEMP\small.log"; Length = 10MB; LastWriteTime =
                (Get-Date).AddDays(-10) })
            }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\]'
        ...
        It "Ignores recent temp files regardless of size" {
            function Get-CimInstance { }
            Mock Get-CimInstance { @() }
            Mock Get-ChildItem {
@([pscustomobject]@{ FullName = "$env:TEMP\fresh.bin"; Length = 5GB; LastWriteTime =
                (Get-Date).AddHours(-1) })
            }
            $out = Main *>&1
            $out | Where-Object { $_ -is [int] } | Should -Be 0
