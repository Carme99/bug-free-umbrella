...
Describe 'Invoke-RemediationFixWindowsUpdateRebootPending' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationFixWindowsUpdateRebootPending.ps1'
        $scriptText = Get-Content $scriptPath -Raw

        # Safe: the script's top-level guard skips Main when dot-sourced (spec §3).
        . $scriptPath

        # No Windows-only cmdlets are used: shutdown.exe is reached only through the
        # Invoke-Shutdown wrapper seam, which is mocked below.
    }
    ...
    Context 'Behavior' {
        It 'Schedules a restart when a reboot-pending flag is present' {
            Mock Test-Path { $true } -ParameterFilter { $Path -like '*Component Based Servicing\RebootPending' }
            Mock Invoke-Shutdown { 0 }

            $out = Main *>&1

            ($out | Out-String) | Should -Match '\[\+\] Reboot pending state detected; scheduled a restart in 15 minutes'
            Should -Invoke Invoke-Shutdown -Times 1 -ParameterFilter { $args -contains '/r' -and $args -contains '900' }
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Schedules a restart from the WindowsUpdate RebootRequired flag alone' {
            Mock Test-Path { $true } -ParameterFilter { $Path -like '*Auto Update\RebootRequired' }
            Mock Invoke-Shutdown { 0 }

            $out = Main *>&1

            ($out | Out-String) | Should -Match '\[\+\] Reboot pending state detected'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Is idempotent: no pending flags returns 0 without scheduling anything' {
            Mock Test-Path { $false }
            Mock Invoke-Shutdown { 0 }

            $out = Main *>&1

            ($out | Out-String) | Should -Match '\[\+\] Already converged: no reboot pending flags present'
            Should -Not -Invoke Invoke-Shutdown
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 1 and writes [-] prefixed output when shutdown.exe fails' {
            Mock Test-Path { $true } -ParameterFilter { $Path -like '*Component Based Servicing\RebootPending' }
            Mock Invoke-Shutdown { 1 }
        ...
        It 'Honours -WhatIf: no restart is scheduled' {
            Mock Test-Path { $true } -ParameterFilter { $Path -like '*Component Based Servicing\RebootPending' }
            Mock Invoke-Shutdown { 0 }
            ...
            try {
                Main *>&1 | Out-Null
                Should -Not -Invoke Invoke-Shutdown -Because '-WhatIf must suppress scheduling a restart'
            }
            finally {
