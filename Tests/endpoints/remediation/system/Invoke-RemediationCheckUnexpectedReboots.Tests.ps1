...
Describe 'Invoke-RemediationCheckUnexpectedReboots' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckUnexpectedReboots.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckUnexpectedReboots\.ps1'
    ...
    Context 'Behavior' {
        It 'Logs the timestamped reboot diagnostics and returns 0' {
            Mock Get-Date { [datetime]'2026-08-23T12:00:00' }

            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\*\] Logged: 08/23/2026 12:00:00'
            ($out | Out-String) | Should -Match '\[!\] This device has been flagged for IT investigation'
            ($out | Out-String) | Should -Match '\[\+\] Diagnostics logged for IT tracking'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Is idempotent: re-running changes nothing and still returns 0' {
            # Pin the timestamp so both runs produce identical log lines.
            Mock Get-Date { [datetime]'2026-08-23T12:00:00' }
