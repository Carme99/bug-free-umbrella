...
Describe 'Invoke-RemediationCheckWindowsActivationGracePeriod' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckWindowsActivationGracePeriod.ps1'
        ...
        # The stubs declare their parameters so -ParameterFilter can route calls.
            ...
            Set-Item -Path 'Function:global:Invoke-CimMethod' -Value {
                ...
                throw 'Invoke-CimMethod is not available on this platform'
            }

        # Native executables are mocked via their wrapper functions, never by name.
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckWindowsActivationGracePeriod\.ps1'
        ...
        It 'Declares SupportsShouldProcess for its licensing state changes' {
    ...
    Context 'Behavior' {
        It 'Triggers activation via the cscript wrapper and refreshes license status' {
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] Triggered Windows activation'
            ($out | Out-String) | Should -Match '\[\+\] Refreshed license status'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
            Should -Invoke Invoke-Cscript -Times 1 -Exactly
            Should -Invoke Invoke-CimMethod -Times 1 -Exactly -ParameterFilter { $MethodName -eq 'RefreshLicenseStatus' }
        }

        It 'Warns without failing when the activation attempt reports a non-zero exit code' {
            Mock Invoke-Cscript { 1 }

            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\] Activation initiated \(may require additional time\)'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Is idempotent: already-activated device returns 0 and skips the refresh gracefully' {
            Mock Get-CimInstance { $null }   # licensing service unavailable on re-check
        ...
        It 'Returns 1 and writes [-] prefixed output when the license refresh fails' {
            Mock Invoke-CimMethod { throw "COM gone" }
        ...
        It 'Honours -WhatIf: neither activation nor license refresh runs' {
            # Main is an advanced function carrying SupportsShouldProcess.
            $WhatIfPreference = $true
            try {
                Main *>&1 | Out-Null
                Should -Invoke Invoke-Cscript -Times 0 -Exactly -Because '-WhatIf must suppress activation'
                Should -Invoke Invoke-CimMethod -Times 0 -Exactly -Because '-WhatIf must suppress the refresh'
            }
            finally {
