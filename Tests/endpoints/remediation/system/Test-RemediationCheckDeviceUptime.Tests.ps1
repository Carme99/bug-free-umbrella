...
Describe 'Test-RemediationCheckDeviceUptime' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckDeviceUptime.ps1'
    ...
    Context 'Behavior' {
        It 'Returns 0 when uptime is within limits and no reboot is pending (compliant)' {
            Mock Get-CimInstance { [pscustomobject]@{ LastBootUpTime = (Get-Date).AddDays(-2) } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\]'
            ($out | Out-String) | Should -Match 'Device uptime is within acceptable limits'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
            Should -Invoke Get-CimInstance -Times 1 -Exactly
        }

        It 'Returns 1 when uptime exceeds the reboot threshold (non-compliant)' {
            Mock Get-CimInstance { [pscustomobject]@{ LastBootUpTime = (Get-Date).AddDays(-20) } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'running for \d+(\.\d+)? days \(threshold: 14 days\)'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }

        It 'Returns 1 when a Windows Update reboot is pending (non-compliant)' {
            Mock Get-CimInstance { [pscustomobject]@{ LastBootUpTime = (Get-Date).AddDays(-2) } }
            Mock Test-Path {
                param($Path)
            ...
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'Windows Update reboot pending'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }

        It 'Returns 1 when boot time cannot be retrieved (non-compliant)' {
            # Baseline Get-CimInstance mock returns $null -> "Unable to retrieve system boot time".
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'Unable to retrieve system boot time'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }
