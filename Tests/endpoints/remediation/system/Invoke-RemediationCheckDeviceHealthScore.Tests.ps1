...
Describe 'Invoke-RemediationCheckDeviceHealthScore' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckDeviceHealthScore.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckDeviceHealthScore\.ps1'
    ...
    Context 'Behavior' {
        It 'Prints the improvement plan and returns 0' {
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\*\] Device Health Improvement Plan:'
            ($out | Out-String) | Should -Match 'PRIORITY 1: CRITICAL ISSUES'
            ($out | Out-String) | Should -Match 'MONITORING & REPORTING'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Reports completion with the success prefix on every run (idempotent)' {
            $first = Main *>&1
            $second = Main *>&1
            ($first | Out-String) | Should -Match '\[\+\]'
            ($second | Out-String) | Should -Match '\[\+\]'
            $second | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 1 and writes [-] prefixed output when plan generation fails' {
