...
Describe 'Invoke-RemediationCheckSystemEventErrors' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckSystemEventErrors.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckSystemEventErrors\.ps1'
    ...
    Context 'Behavior' {
        It 'Prints the prioritised guidance and returns 0' {
            Mock Get-Date { [datetime]'2026-08-23T12:00:00' }

            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\*\] Priority actions based on error type:'
            ($out | Out-String) | Should -Match 'Disk errors \(Event IDs 7, 11, 51, 153, 55\)'
            ($out | Out-String) | Should -Match '\[\*\] Logged: 08/23/2026 12:00:00'
            ($out | Out-String) | Should -Match '\[!\] URGENT: Device flagged for immediate IT investigation'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Is idempotent: re-running changes nothing and still returns 0' {
            Mock Get-Date { [datetime]'2026-08-23T12:00:00' }

            $first = Main *>&1
            $second = Main *>&1
            $second | Where-Object { $_ -is [int] } | Should -Be 0
            ($first | Out-String) | Should -Be (($second | Out-String))
        }

        It 'Returns 1 and writes [-] prefixed output when the timestamp fails' {
            Mock Get-Date { throw "clock unavailable" }
