...
Describe 'Invoke-RemediationCheckDeviceUptime' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckDeviceUptime.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckDeviceUptime\.ps1'
    ...
    Context 'Behavior' {
        It 'Returns 0 and reports compliance when uptime is low and no reboot pending' {
            Mock Get-CimInstance {
                [pscustomobject]@{ LastBootUpTime = (Get-Date).AddDays(-3) }
            }
            $out = Main *>&1
        ...
        It 'Flags pending reboots and excessive uptime but still returns 0' {
            Mock Get-CimInstance {
                [pscustomobject]@{ LastBootUpTime = (Get-Date).AddDays(-30) }
            }
            Mock Test-Path { $true }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'exceeds the 14 day threshold'
            ($out | Out-String) | Should -Match 'Pending reboot detected for:'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 0 with a warning when no operating system data is available (idempotent)' {
            Mock Get-CimInstance { $null }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\] Unable to determine uptime'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 1 and writes [-] prefixed output when the uptime query fails' {
