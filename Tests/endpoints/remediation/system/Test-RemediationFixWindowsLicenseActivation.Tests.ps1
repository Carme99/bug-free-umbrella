...
Describe "Test-RemediationFixWindowsLicenseActivation" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/system/Test-RemediationFixWindowsLicenseActivation.ps1'
    ...
    Context "Behavior" {
        It "Returns 0 when slmgr reports the license as Licensed" {
            Mock Invoke-WindowsActivationQuery {
                [pscustomobject]@{ ExitCode = 0; Output = "Name: Windows`nLicense Status: Licensed" }
            }
        ...
        It "Returns 1 and writes [-] prefixed output when the slmgr query fails" {
            Mock Invoke-WindowsActivationQuery { [pscustomobject]@{ ExitCode = 1; Output = '' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[-\]'
            $out | Where-Object { $_ -is [int] } | Should -Be 1
        }
