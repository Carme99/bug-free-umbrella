...
Describe 'Invoke-RemediationCheckHardwareErrors' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckHardwareErrors.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckHardwareErrors\.ps1'
    ...
    Context 'Behavior' {
        It 'Prints the critical guidance and returns 0' {
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\] CRITICAL: Hardware Error Remediation'
            ($out | Out-String) | Should -Match 'IMMEDIATE ACTIONS REQUIRED'
            ($out | Out-String) | Should -Match 'WARRANTY CHECK'
        ...
        It 'Returns 1 and writes [-] prefixed output when guidance generation fails' {
