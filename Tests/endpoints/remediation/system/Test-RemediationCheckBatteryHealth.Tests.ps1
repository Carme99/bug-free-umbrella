...
Describe 'Test-RemediationCheckBatteryHealth' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckBatteryHealth.ps1'
        ...
        # Contain the run: temp report lands in the Pester test drive.
        $env:TEMP = "$TestDrive"
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Test-RemediationCheckBatteryHealth\.ps1'
        ...
        It 'Documents its detect exit-code contract in DESCRIPTION' {
            ...
            $scriptText | Should -Match '(?s)Exit codes?: 0 =.+?degradation\s+detected'
        }
    ...
    Context 'Behavior' {
        It 'Returns 0 when no battery is present (not applicable)' {
        ...
        It 'Returns 1 when report shows capacity below threshold' {
            Mock Get-CimInstance { [pscustomobject]@{ Status = 'OK' } }
            Mock Invoke-PowerCfg { 0 }
            Mock Test-Path { $true }
            Mock Get-Content {
                [xml]'<BatteryReport><Batteries><Battery DesignCapacity="50000" FullChargeCapacity="30000" /></Batteries></BatteryReport>'
            }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
        ...
        It 'Returns 0 when battery is healthy (idempotent re-check)' {
            Mock Get-CimInstance { [pscustomobject]@{ Status = 'OK' } }
            Mock Invoke-PowerCfg { 0 }
            Mock Test-Path { $true }
            Mock Get-Content {
                [xml]'<BatteryReport><Batteries><Battery DesignCapacity="50000" FullChargeCapacity="45000" /></Batteries></BatteryReport>'
            }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] Battery health is acceptable'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }
