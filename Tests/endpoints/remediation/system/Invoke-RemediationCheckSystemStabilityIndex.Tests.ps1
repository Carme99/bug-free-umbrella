...
Describe 'Invoke-RemediationCheckSystemStabilityIndex' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckSystemStabilityIndex.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckSystemStabilityIndex\.ps1'
    ...
    Context 'Behavior' {
        It 'Prints the stability recommendations and returns 0' {
            Mock Get-Date { [datetime]'2026-08-23T12:00:00' }

            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\*\] Common causes and solutions:'
            ($out | Out-String) | Should -Match '\[\*\] Logged: 08/23/2026 12:00:00'
            ($out | Out-String) | Should -Match '\[!\] Device flagged for IT stability review'
            ($out | Out-String) | Should -Match '\[\+\] Recommendations logged for IT tracking'
