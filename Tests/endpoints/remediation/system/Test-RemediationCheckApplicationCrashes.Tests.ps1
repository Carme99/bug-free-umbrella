...
Describe 'Test-RemediationCheckApplicationCrashes' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Test-RemediationCheckApplicationCrashes.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Test-RemediationCheckApplicationCrashes\.ps1'
    ...
    Context 'Behavior' {
        It 'Returns 0 when no significant crashes are detected (compliant)' {
            Mock Test-Path { $false }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\]'
            ($out | Out-String) | Should -Match 'No significant application crashes detected'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
        }

        It 'Returns 1 when crash count exceeds the maximum (non-compliant)' {
            $crashEvents = 1..11 | ForEach-Object {
                [pscustomobject]@{ Properties = @([pscustomobject]@{ Value = "app$($_ % 3).exe" }) }
            }
            Mock Get-WinEvent {
                param($FilterHashtable)
                if ($FilterHashtable['ID'] -eq 1000) { $crashEvents }
                else { $null }
            }
            Mock Test-Path { $false }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'Excessive application crashes detected: 11 events'
