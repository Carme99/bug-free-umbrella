...
Describe 'Invoke-RemediationCheckBootPerformance' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationCheckBootPerformance.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationCheckBootPerformance\.ps1'
    ...
    Context 'Behavior' {
        It 'Prints the recommendations and returns 0' {
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\*\] Boot Performance Remediation:'
            ($out | Out-String) | Should -Match 'Optimization recommendations'
        ...
        It 'Returns 1 and writes [-] prefixed output when report generation fails' {
