...
Describe 'Invoke-RemediationFixOutdatedDrivers' {
    BeforeAll {
        ...
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/remediation/system/Invoke-RemediationFixOutdatedDrivers.ps1'
    ...
    Context 'Help & Metadata' {
        It 'Declares the five required .NOTES fields with relaunch values' {
            $scriptText | Should -Match 'File Name\s*:\s*Invoke-RemediationFixOutdatedDrivers\.ps1'
        ...
        It 'Declares SupportsShouldProcess for its driver mutations' {
    ...
    Context 'Behavior' {
        It 'Installs pending updates and rescans problem devices via pnputil, returning 0' {
            Mock Install-DriverUpdates { 'Successfully installed 2 driver update(s)' }
            Mock Get-CimInstance {
                @([pscustomobject]@{ Name = 'Video'; ConfigManagerErrorCode = 28 })
            }
            Mock Invoke-PnpUtil { 0 }

            $out = Main *>&1
            ($out | Out-String) | Should -Match 'Successfully installed 2 driver update\(s\)'
            ($out | Out-String) | Should -Match 'Triggered hardware rescan for problem devices'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
            Should -Invoke Invoke-PnpUtil -Times 1 -Exactly -Scope It
        }

        It 'Is idempotent: converged system returns 0 with no changes' {
            Mock Install-DriverUpdates { $null }
            Mock Invoke-PnpUtil { 0 }
            Mock Get-CimInstance {
                @([pscustomobject]@{ Name = 'Healthy device'; ConfigManagerErrorCode = 0 })
            }

            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[\+\] Already clean'
            $out | Where-Object { $_ -is [int] } | Should -Be 0
            Should -Invoke Invoke-PnpUtil -Times 0 -Exactly -Scope It
        }

        It 'Returns 1 and writes [-] prefixed output when device enumeration fails' {
            Mock Install-DriverUpdates { $null }
        ...
        It 'Honours -WhatIf: nothing is installed and no hardware rescan runs' {
            Mock Install-DriverUpdates { 'Successfully installed 2 driver update(s)' }
            Mock Invoke-PnpUtil { 0 }
            Mock Get-CimInstance {
                @([pscustomobject]@{ Name = 'Video'; ConfigManagerErrorCode = 28 })
            ...
            try {
                Main *>&1 | Out-Null
                Should -Invoke Install-DriverUpdates -Times 0 -Exactly -Scope It -Because '-WhatIf must suppress the COM install pipeline'
                Should -Invoke Invoke-PnpUtil -Times 0 -Exactly -Scope It -Because '-WhatIf must suppress the hardware rescan'
            }
            finally {
