...
Describe "Invoke-WingetNotepadPlusPlus" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/productivity/NotepadPlusPlus'
            $scriptRelPath += '/Invoke-WingetNotepadPlusPlus.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetNotepadPlusPlus\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Updates and returns 0 when an update is available and notepad++.exe is not running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-WinGetPackage {
                ...
                if ($script:calls -eq 1) {
                    [pscustomobject] @{
                        Name              = 'Notepad++'
                        InstalledVersion  = '8.6.9'
                        AvailableVersions = @('8.6.9', '8.7.0')
        ...
        It "Defers with exit 1 when notepad++.exe is running (open documents never lost)" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'notepad++.exe' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
        ...
        It "Returns 1 via the CLI fallback when notepad++.exe is running" {
