...
Describe "Invoke-WingetRemoteDesktop" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/productivity/RemoteDesktop'
            $scriptRelPath += '/Invoke-WingetRemoteDesktop.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetRemoteDesktop\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Updates and returns 0 when an update is available and no RDP session is running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
        ...
        It "Defers with exit 1 when the Microsoft.RemoteDesktop process is running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'Microsoft.RemoteDesktop' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
        ...
        It "Returns 1 via the CLI fallback when the Microsoft.RemoteDesktop process is running" {
