...
Describe "Invoke-WingetAdobeReader64bit" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/productivity/AdobeReader64bit'
            $scriptRelPath += '/Invoke-WingetAdobeReader64bit.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetAdobeReader64bit\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Is idempotent: already-up-to-date package returns 0 with no update call" {
            ...
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    ...
                    InstalledVersion  = '24.003.20'
                    AvailableVersions = @('24.003.20')
        ...
        It "Updates and returns 0 when an update is available and AcroRd64 is not running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-WinGetPackage {
                ...
                if ($script:calls -eq 1) {
                    [pscustomobject] @{
                        Name              = 'Adobe Acrobat Reader (64-bit)'
                        InstalledVersion  = '24.003.10'
                        AvailableVersions = @('24.003.10', '24.003.20')
        ...
        It "Defers with exit 1 when AcroRd64 is running (open documents never lost)" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'AcroRd64' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
        ...
        It "Skips the update under -WhatIf and still returns 0 (ShouldProcess gate)" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
        ...
        It "Returns 1 via the CLI fallback when AcroRd64 is running" {
            Mock Invoke-WingetWithRetry {
                @(
                    'Name                Version Available'
                    '-------------------- ------- ---------'
                    "$($ID) 24.003.10    24.003.20"
                ) -join "`n"
            }
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'AcroRd64' } }
            $out = Main *>&1
