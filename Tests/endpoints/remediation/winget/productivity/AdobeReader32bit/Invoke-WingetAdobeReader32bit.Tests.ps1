...
Describe "Invoke-WingetAdobeReader32bit" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/productivity/AdobeReader32bit'
            $scriptRelPath += '/Invoke-WingetAdobeReader32bit.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetAdobeReader32bit\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Updates and returns 0 when an update is available and AcroRd32 is not running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
        ...
        It "Defers with exit 1 when AcroRd32 is running (open documents never lost)" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'AcroRd32' } }
            $out = Main *>&1
            ($out | Out-String) | Should -Match '\[!\]'
        ...
        It "Returns 1 via the CLI fallback when AcroRd32 is running" {
