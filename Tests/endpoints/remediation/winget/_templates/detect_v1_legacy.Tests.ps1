...
Describe "detect_v1_legacy.ps1" {
    BeforeAll {
        # Mirrored layout: walk up ../../../../../../ levels from this test folder to the repository root.
        $repoRoot = (Get-Item (Join-Path $PSScriptRoot "../../../../../")).FullName
        $scriptPath = Join-Path $repoRoot "scripts/endpoints/remediation/winget/_templates/detect_v1_legacy.ps1"
    ...
    Context "Help & Metadata" {
        ...
        It "Declares File Name matching the on-disk filename with no orphaned parameters documented" {
            $raw = Get-Content -Path $scriptPath -Raw
            ($raw -match "(?m)^\s*File Name\s*:\s*detect_v1_legacy\.ps1\s*$") | Should -BeTrue
        ...
        It "Marks the superseded generation as deprecated in the synopsis" {
            $raw = Get-Content -Path $scriptPath -Raw
            ($raw -match '(?ms)^\.SYNOPSIS.*DEPRECATED: use detect_v3\.ps1') | Should -BeTrue
        }
    }
    ...
    Context "Behavior" {
        It "Detects an available update via Microsoft.WinGet.Client while the app runs and returns 1" {
            ...
            Mock Get-Process { [pscustomobject]@{ ProcessName = 'PROCESS' } }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 1
            ($out | Out-String) | Should -Match '\[!\]'
            ($out | Out-String) | Should -Match 'currently running'
        }

        It "Detects an available update via Microsoft.WinGet.Client with the app stopped and returns 1" {
            Mock Get-Module { [pscustomobject]@{ Name = 'Microsoft.WinGet.Client' } }
            Mock Import-Module { }
            Mock Get-Command -ParameterFilter { $Name -eq 'Get-WinGetPackage' } {
                [pscustomobject]@{ Name = 'Get-WinGetPackage' }
            ...
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 1
            ($out | Out-String) | Should -Not -Match 'currently running'
        }
        ...
        It "Returns 1 with [-] prefixed output when detection fails" {
            Mock Get-Module { $null }
