...
Describe "remediate_v3_force_close.ps1" {
    BeforeAll {
        # Mirrored layout: walk up five levels from this test folder to the repository root.
        $repoRoot = (Get-Item (Join-Path $PSScriptRoot "../../../../../")).FullName
        $relative = "scripts/endpoints/remediation/winget/_templates/remediate_v3_force_close.ps1"
    ...
    Context "Help & Metadata" {
        ...
        It "Declares File Name matching the on-disk filename with no orphaned parameters documented" {
            $raw = Get-Content -Path $scriptPath -Raw
            ($raw -match "(?m)^\s*File Name\s*:\s*remediate_v3_force_close\.ps1\s*$") | Should -BeTrue
    ...
    Context "Behavior" {
        It "Returns 0 on an already-current system without stopping any process (idempotent)" {
            ...
            Mock Get-WinGetPackage {
                [pscustomobject]@{ Name = 'OBS Studio'; InstalledVersion = '30.0'; IsUpdateAvailable = $false }
            }
            Main *>&1 | Out-Null
        ...
        It "Notifies users, force closes the running app behind ShouldProcess, and returns 0" {
            Mock Get-Module { $true }
            Mock Get-WinGetPackage {
                [pscustomobject]@{
                    Name = 'OBS Studio'
                    InstalledVersion = '30.0'
                    AvailableVersions = @('31.0')
                    IsUpdateAvailable = $true
                }
            }
            $NotifyUserBeforeClose = $true
            Mock Get-InteractiveUserSession { @(' jack console 1') }
            Mock Send-UserSessionMessage { 0 }
            $script:gpCalls = 0
            Mock Get-Process {
                $script:gpCalls++
                if ($script:gpCalls -le 2) { @([pscustomobject]@{ Id = 99 }) } else { $null }
            }
            Main *>&1 | Out-Null
            Should -Invoke Get-InteractiveUserSession -Times 1 -Exactly
            Should -Invoke Send-UserSessionMessage -Times 1 -Exactly
        ...
        It "Falls back to the winget.exe wrapper seam and upgrades through it when the module is missing" {
            Mock Invoke-WingetCommand {
                @('Name Id Version Available Source', '---- -- ------- --------- ------',
                    'OBSStudio WINGETID 1.0.0 2.0.0 winget')
