...
Describe "Invoke-WingetLenovoSystemUpdate" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/vendor-specific/LenovoSystemUpdate/Invoke-WingetLenovoSystemUpdate.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetLenovoSystemUpdate\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Skips with exit 1 and no update while LenSysUpdatez is running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            Mock Get-Module { $wingetModuleStub } `
                -ParameterFilter { $ListAvailable -and $Name -eq 'Microsoft.WinGet.Client' }
            Mock Invoke-WingetWithRetry { throw "unexpected winget.exe CLI call" }
            Mock Get-Process { [pscustomobject] @{ Id = 555; ProcessName = 'LenSysUpdatez' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name              = 'Lenovo System Update'
        ...
        It "Installs the update and returns 0 when verified" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
        ...
        It "Returns 1 with [-] prefixed output when verification after update fails" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
