...
Describe "Invoke-WingetLenovoDockManager" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/vendor-specific/LenovoDockManager/Invoke-WingetLenovoDockManager.ps1'
    ...
    Context "Behavior" {
        ...
        It "Skips with exit 1 and no update while LenovoDockMgr is running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            Mock Get-Module { $wingetModuleStub } `
                -ParameterFilter { $ListAvailable -and $Name -eq 'Microsoft.WinGet.Client' }
            Mock Invoke-WingetWithRetry { throw "unexpected winget.exe CLI call" }
            Mock Get-Process { [pscustomobject] @{ Id = 555; ProcessName = 'LenovoDockMgr' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name              = 'Lenovo Dock Manager'
