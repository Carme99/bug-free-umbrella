...
Describe "Invoke-WingetCpp2013RedistX64" {
    BeforeAll {
        ...
        $scriptRelPath = Join-Path 'scripts/endpoints/remediation/winget' `
            'runtimes/Cpp2013Redist-x64/Invoke-WingetCpp2013RedistX64.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetCpp2013RedistX64.ps1'
        ...
        It "Documents the exact package ID Microsoft.VCRedist.2013.x64 in help and configuration" {
            $raw = Get-Content -Path $scriptPath -Raw
            $raw | Should -Match ([regex]::Escape("Microsoft.VCRedist.2013.x64"))
        }
    ...
    Context "Behavior" {
        ...
        It "Skips the update and returns 1 while the redistributable process is running" {
            ...
            Mock Get-Module { $wingetModuleStub } `
                -ParameterFilter { $ListAvailable -and $Name -eq 'Microsoft.WinGet.Client' }
            Mock Invoke-WingetWithRetry { throw "unexpected winget.exe CLI call" }
            Mock Get-Process { [pscustomobject] @{ Id = 4321; ProcessName = 'Microsoft.VCRedist.2013.x64' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name              = 'Microsoft.VCRedist.2013.x64'
            ...
            Mock Invoke-WingetWithRetry {
                ...
                if ($script:wCalls -eq 1) {
                    @(
                        ...
                        '-------------------- ------- ---------'
                        "Microsoft.VCRedist.2013.x64 14.0.1    14.0.2"
                    ) -join "`n"
                }
                elseif ($script:wCalls -eq 2) { '' }
                    ...
                    @(
                        'Name                Version',
                        '-------------------- -------',
                        "Microsoft.VCRedist.2013.x64 14.0.2"
                    ) -join "`n"
                }
            }
            Mock Get-Process { $null }
