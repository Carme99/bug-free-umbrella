...
Describe "Test-WingetOBS" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/media/OBS/Test-WingetOBS.ps1'
    ...
    Context "Behavior" {
        ...
        It "Uses the CLI fallback when the module probe fails and reports not installed as compliant" {
            # No module-cmdlet stubs here: Get-Module probe fails on Linux and Get-Command does not
