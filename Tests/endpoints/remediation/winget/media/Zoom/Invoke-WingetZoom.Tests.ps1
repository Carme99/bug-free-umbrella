...
Describe "Invoke-WingetZoom" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/media/Zoom/Invoke-WingetZoom.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetZoom\.ps1'
        ...
        It "Declares one .PARAMETER per param() parameter" {
            ...
            $raw = Get-Content -Path $scriptPath -Raw
            $matchesParam = [regex]::Matches($raw, '(?m)^\s*\.PARAMETER\s+(\S+)')
            $documented = @($matchesParam | ForEach-Object { $_.Groups[1].Value })
    ...
    Context "Behavior" {
        ...
        It "Defers with exit 1 when the app process is running" {
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'zoom' } }
