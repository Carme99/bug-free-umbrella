...
Describe "Invoke-WingetVLCMediaPlayer" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/media/VLCMediaPlayer/Invoke-WingetVLCMediaPlayer.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetVLCMediaPlayer\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Updates and returns 0 when an update is available and vlc is not running" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-WinGetPackage {
                ...
                if ($script:calls -eq 1) {
                    [pscustomobject] @{
                        Name                 = 'VLC media player'
                        InstalledVersion     = '3.0.20'
                        AvailableVersions    = @('3.0.20', '3.0.21')
        ...
        It "Defers with exit 1 when vlc is running (playback never interrupted)" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'vlc' } }
        ...
        It "Returns 1 via the CLI fallback when vlc is running" {
