...
Describe "Test-WingetFirefox" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/browsers/Firefox/Test-WingetFirefox.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Test-WingetFirefox\.ps1'
    ...
    Context "Syntax & Static" {
        ...
        It "Does not hardcode the connectivity host outside the config variable" {
            $raw = Get-Content -Path $scriptPath -Raw
            ($raw -split "`n" | Where-Object { $_ -match 'www\.microsoft\.com' }).Count | Should -Be 1
            $raw | Should -Match '\$ConnectivityHost'
        }
    }
    ...
    Context "Behavior" {
        It "Returns 1 when an update is available via the module path" {
            ...
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    ...
                    InstalledVersion     = '128.0'
                    AvailableVersions    = @('128.0', '129.0')
        ...
        It "Is idempotent: up-to-date package returns 0 with no update attempt" {
            ...
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name                 = 'Mozilla Firefox'
                    InstalledVersion     = '129.0'
                    AvailableVersions    = @('129.0')
        ...
        It "Returns 0 without probing winget when network connectivity is unavailable" {
        ...
        It "Uses the CLI fallback when the module probe fails and reports not installed as compliant" {
            # No module stubs here: Get-Module probe fails on Linux and Get-Command does not
