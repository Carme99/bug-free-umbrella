...
Describe "Invoke-WingetFirefox" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/browsers/Firefox/Invoke-WingetFirefox.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetFirefox\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Defers with exit 1 when the app process is running" {
            ...
            Mock Get-Process { [pscustomobject] @{ ProcessName = 'firefox' } }
            Main *>&1 | Where-Object { $_ -is [int] } | Should -Be 1
            Should -Invoke Update-WinGetPackage -Times 0 -Exactly
        }

        It "Returns 0 without updating when the package is not installed" {
            # Stub module surface inline so Pester can Mock these commands.
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
