...
Describe "Test-WingetPowerShell7" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/development/PowerShell7/Test-WingetPowerShell7.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Test-WingetPowerShell7\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Reports [-] prefixed output but stays compliant (exit 0) on upstream errors" {
            function Get-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            Mock Get-Module { $wingetModuleStub } `
                -ParameterFilter { $ListAvailable -and $Name -eq 'Microsoft.WinGet.Client' }
