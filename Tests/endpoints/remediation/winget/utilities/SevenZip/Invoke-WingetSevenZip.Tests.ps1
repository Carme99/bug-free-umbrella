...
Describe "Invoke-WingetSevenZip" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/utilities/SevenZip/Invoke-WingetSevenZip.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetSevenZip\.ps1'
    ...
    Context "Behavior" {
        It "Returns 0 when the package is not installed and never closes or updates anything" {
        ...
        It "Is idempotent: already-up-to-date package returns 0 with no close or update activity" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name              = '7-Zip'
                    InstalledVersion  = '24.7.0'
                    AvailableVersions = @('24.7.0')
            ...
            Should -Invoke Stop-Process -Times 0 -Exactly -Because "the app is not running"
        }

        It "Force closes a running app, installs the update and returns 0" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Get-Process {
                $script:procCalls++
                # Still running for the initial check and the first close attempt, gone afterwards.
                if ($script:procCalls -le 2) { [pscustomobject] @{ Id = 555; ProcessName = '7zFM' } } else { $null }
            }
            $script:calls = 0
            Mock Get-WinGetPackage {
                $script:calls++
                if ($script:calls -eq 1) {
                    [pscustomobject] @{
                        Name              = '7-Zip'
                        InstalledVersion  = '24.6.0'
                        AvailableVersions = @('24.6.0', '24.7.0')
        ...
        It "Aborts with exit 1 and no update when the process cannot be closed" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Mock Stop-Process { }
            Mock Get-LoggedOnUserSession { @() }
            Mock Get-Process { [pscustomobject] @{ Id = 555; ProcessName = '7zFM' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name              = '7-Zip'
                    InstalledVersion  = '24.6.0'
        ...
        It "Suppresses the force close under -WhatIf and aborts without updating (ShouldProcess gate)" {
            function Get-WinGetPackage { }
            function Update-WinGetPackage { }
            Mock Update-WinGetPackage { }
            $wingetModuleStub = [pscustomobject] @{ Name = 'Microsoft.WinGet.Client' }
            ...
            Main -WhatIf *> $null
            Should -Invoke Stop-Process -Times 0 -Exactly -Because "-WhatIf must suppress the force close"
            Should -Invoke Update-WinGetPackage -Times 0 -Exactly -Because "-WhatIf must suppress the update"
        }

        It "Performs the full CLI-fallback close-and-upgrade flow and returns 0" {
            $script:wCalls = 0
