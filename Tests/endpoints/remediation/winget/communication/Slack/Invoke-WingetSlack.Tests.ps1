...
Describe "Invoke-WingetSlack" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/communication/Slack/Invoke-WingetSlack.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetSlack\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Is idempotent: already-up-to-date package returns 0 with no close/update activity" {
            ...
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name                 = 'Slack'
                    InstalledVersion     = '4.41.0'
                    AvailableVersions    = @('4.41.0')
        ...
        It "Notifies, force closes a running app, updates and returns 0" {
            ...
            Mock Get-Process {
                $script:procCalls++
                if ($script:procCalls -eq 1) { [pscustomobject] @{ Id = 555; ProcessName = 'Slack' } } else { $null }
            }
            $script:calls = 0
            Mock Get-WinGetPackage {
                $script:calls++
                if ($script:calls -eq 1) {
                    [pscustomobject] @{
                        Name                 = 'Slack'
                        InstalledVersion     = '4.40.0'
                        AvailableVersions    = @('4.40.0', '4.41.0')
        ...
        It "Aborts with exit 1 and no update when the process cannot be closed" {
            ...
            Mock Invoke-WingetWithRetry { throw "unexpected winget.exe CLI call" }
            Mock Get-LoggedOnUserSession { @() }
            Mock Get-Process { [pscustomobject] @{ Id = 555; ProcessName = 'Slack' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name                 = 'Slack'
                    InstalledVersion     = '4.40.0'
