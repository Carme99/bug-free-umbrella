...
Describe "Invoke-WingetDiscord" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/winget/communication/Discord/Invoke-WingetDiscord.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 2.0.0 and Date 2026-09-16" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-WingetDiscord\.ps1'
    ...
    Context "Behavior" {
        ...
        It "Leaves a running app alone when no update is available" {
            ...
            Mock Invoke-WingetWithRetry { throw "unexpected winget.exe CLI call" }
            Mock Get-LoggedOnUserSession { @() }
            Mock Get-Process { [pscustomobject] @{ Id = 4321; ProcessName = 'Discord' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name              = 'Discord'
                    InstalledVersion  = '1.0.9200'
        ...
        It "Notifies, force closes a running app, updates and returns 0" {
            ...
            Mock Get-Process {
                $script:procCalls++
                if ($script:procCalls -eq 1) { [pscustomobject] @{ Id = 4242; ProcessName = 'Discord' } } else { $null }
            }
            $script:calls = 0
            Mock Get-WinGetPackage {
                $script:calls++
                if ($script:calls -eq 1) {
                    [pscustomobject] @{
                        Name                 = 'Discord'
                        InstalledVersion     = '1.0.9100'
                        AvailableVersions    = @('1.0.9100', '1.0.9200')
        ...
        It "Aborts with exit 1 and no update when the process cannot be closed" {
            ...
            Mock Invoke-WingetWithRetry { throw "unexpected winget.exe CLI call" }
            Mock Get-LoggedOnUserSession { @() }
            Mock Get-Process { [pscustomobject] @{ Id = 4242; ProcessName = 'Discord' } }
            Mock Get-WinGetPackage {
                [pscustomobject] @{
                    Name                 = 'Discord'
                    InstalledVersion     = '1.0.9100'
        ...
        It "Performs the full CLI-fallback upgrade and returns 0 when the process is not running" {
            ...
            Mock Invoke-WingetWithRetry {
                ...
                '-------------------- ------- ---------'
                "$($ID) 1.0.9100    1.0.9200"
            ) -join "`n"
                }
                elseif ($script:wCalls -eq 2) { '' }
                else {
                    @('Name                Version', '-------------------- -------', "$($ID) 1.0.9200") -join "`n"
                }
            }
            Mock Get-Process { $null }
            $out = Main *>&1
