...
Describe "Invoke-RemediationCheckOutdatedCriticalAppsPriorityOnly" {
    BeforeAll {
        ...
        $scriptRelPath = 'scripts/endpoints/remediation/security/' +
            'Invoke-RemediationCheckOutdatedCriticalAppsPriorityOnly.ps1'
    ...
    Context "Help & Metadata" {
        It "Declares required .NOTES fields with Version 1.0.0 and Date 2026-08-23" {
            ...
            $raw | Should -Match 'File Name:\s*Invoke-RemediationCheckOutdatedCriticalAppsPriorityOnly\.ps1'
    ...
    Context "Behavior" {
        It "Returns 0 when no priority applications require updates" {
            Mock Invoke-Winget {
                [PSCustomObject]@{
                    Output = @(
                        '  Some Utility    Some.Utility    1.0    2.0',
                        ''
                    )
                    ExitCode = 0
                }
            ...
            $out = Main *>&1
            ($out | Out-String) | Should -Match 'No priority applications require updates'
            ...
            Mock Invoke-Winget {
                [PSCustomObject]@{
                    Output = @(
                        ...
                        '----------------  ---------------  -----------  ---------',
                        '  Mozilla Firefox  Mozilla.Firefox  130.0        131.0',
                        ''
                    )
                    ExitCode = 0
                }
        ...
        It "Retries failed updates up to MaxRetries (2) and returns 1 when all attempts fail" {
            function Start-Job { }
            function Wait-Job { }
            function Receive-Job { }
            function Remove-Job { }
            ...
            Mock Invoke-Winget {
                [PSCustomObject]@{
                    Output = @('  Mozilla Firefox  Mozilla.Firefox  130.0        131.0', '')
                    ExitCode = 0
                }
            }
            Mock Start-Job { [pscustomobject]@{ Id = [guid]::NewGuid() } }
            Mock Wait-Job { $true }
            Mock Receive-Job { 'Installation failed with error 0x80131500' }
            Mock Remove-Job { }
            ...
            Should -Invoke Start-Job -Exactly 2 -Scope It
            Should -Invoke Start-Sleep -Exactly 1 -Scope It
        }

        It "Returns 1 when the winget inventory itself fails" {
            Mock Invoke-Winget {
