...
Describe 'endpoints/devices/proactive-remediations/Check-OutdatedCriticalApps/remediate_priority_only' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../../scripts/endpoints/devices/proactive-remediations/Check-OutdatedCriticalApps/remediate_priority_only.ps1'
    ...
    Context 'Help & Metadata' {
        ...
        It 'Records the actual disk file name in .NOTES' {
            $rawText = Get-Content -Path $scriptPath -Raw
            $rawText | Should -Match "(?m)^\s*File Name\s*:\s*remediate_priority_only.ps1\s*$"
        }
