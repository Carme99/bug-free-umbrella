...
Describe "remediate.ps1" {
    BeforeAll {
        ...
        $scriptPath = Join-Path $repoRoot "scripts/endpoints/devices/proactive-remediations/Fix-TempFiles/remediate.ps1"
    ...
    Context "Behavior" {
        It "Forwards to the canonical script and preserves its exit code" {
            ...
            Should -Invoke Invoke-CanonicalScript -Times 1 -Exactly -ParameterFilter {
                $Path -like '*Invoke-RemediationFixTempFiles.ps1'
            }
        }
