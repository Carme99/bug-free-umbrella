...
Describe 'endpoints/adobe-rum/remediate' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/devices/adobe-rum/remediate.ps1'
...
Mock Stop-Transcript { }
    ...
    Context 'Behavior' {
        It 'Reports installed updates and returns 0' {
            Mock Invoke-RemoteUpdateManager { "Installing...`nFollowing Updates were successfully installed :`n(KBRG/13.0.3.693/win64)" }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 0
            ($out | Out-String) | Should -Match 'successfully installed'
        }

        It 'Is idempotent: reports nothing to do and returns 0 when no updates apply' {
