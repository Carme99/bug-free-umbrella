...
Describe 'endpoints/adobe-rum/detect' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/devices/adobe-rum/detect.ps1'

        # Safe: the script top-level guard skips Main when dot-sourced.
        . $scriptPath

Mock Test-Path { param($Path) return ($Path -like '*RemoteUpdateManager.exe') }
        Mock New-Item { }
        Mock Start-Transcript { }
        Mock Stop-Transcript { }
        Mock Get-Item { [pscustomobject]@{ VersionInfo = [pscustomobject]@{ FileVersion = '3.0.0.8' } } }
    }
    ...
    Context 'Behavior' {
        It 'Returns 1 when RUM reports applicable updates' {
            Mock Invoke-RemoteUpdateManager { "Starting...`nFollowing Updates are applicable on the system :`n(KBRG/13.0.3.693/win64)" }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 1
            ($out | Out-String) | Should -Match 'Updates are required'
        }

        It 'Returns 0 when no updates are applicable' {
            Mock Invoke-RemoteUpdateManager { 'RemoteUpdateManager exiting with Return Code (0)' }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 0
            ($out | Out-String) | Should -Match 'No updates are required'
        }

        It 'Returns 0 with a warning line when RUM is not installed' {
            Mock Test-Path { $false }
            Mock Invoke-RemoteUpdateManager { 'never called' }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 0
            ($out | Out-String) | Should -Match 'Adobe RUM not found'
        }

        It 'Returns 1 and writes [-] prefixed output when the RUM run fails' {
            Mock Invoke-RemoteUpdateManager { throw 'RUM crashed' }
            $out = Main *>&1
