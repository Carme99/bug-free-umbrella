...
Describe "remediate_v2_standard.ps1" {
    BeforeAll {
        # Mirrored layout: walk up five levels from this test folder to the repository root.
        $repoRoot = (Get-Item (Join-Path $PSScriptRoot "../../../../../")).FullName
        $relative = "scripts/endpoints/devices/winget/_templates/remediate_v2_standard.ps1"
    ...
    Context "Help & Metadata" {
        ...
        It "Declares File Name matching the on-disk filename with no orphaned parameters documented" {
            $raw = Get-Content -Path $scriptPath -Raw
            ($raw -match "(?m)^\s*File Name\s*:\s*remediate_v2_standard\.ps1\s*$") | Should -BeTrue
    ...
    Context "Behavior" {
        ...
        It "Auto-detects the app name, upgrades, waits, and verifies before returning 0" {
            Mock Get-Module { $true }
            Mock Get-WinGetPackage {
                [pscustomobject]@{
                    Name = 'Google Chrome'
            ...
            Main *>&1 | Out-Null
            Should -Invoke Write-Host -ParameterFilter {
                ($Object -match 'Google Chrome updated successfully to version 120\.0\.0\.0') -and
                ($Object -match '^\[\+\]')
            }
            Should -Invoke Update-WinGetPackage -Times 1 -Exactly
            Should -Invoke Start-Sleep -Times 1 -Exactly -Because 'the verification wait is a mockable delay'
        }
        ...
        It "Falls back to the winget.exe wrapper seam and upgrades through it when the module is missing" {
            Mock Invoke-WingetCommand {
                @('Name Id Version Available Source', '---- -- ------- --------- ------',
                    'WINGETID Google Chrome 1.0.0 2.0.0 winget')
            } -ParameterFilter { $Arguments[0] -eq 'list' }
            Mock Invoke-WingetCommand { @('') } -ParameterFilter { $Arguments[0] -eq 'upgrade' }
            Main *>&1 | Out-Null
            Should -Invoke Write-Host -ParameterFilter {
                ($Object -match 'Google Chrome updated successfully to version 2\.0\.0') -and
                ($Object -match '^\[\+\]')
            }
        ...
        It "Returns 1 with [-]-prefixed output when the wrapper seam reports failure" {
