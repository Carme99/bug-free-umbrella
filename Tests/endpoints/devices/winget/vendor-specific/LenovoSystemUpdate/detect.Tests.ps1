...
Describe "detect.ps1" {
    BeforeAll {
        ...
        $scriptPath = Join-Path $repoRoot `
            "scripts/endpoints/devices/winget/vendor-specific/LenovoSystemUpdate/detect.ps1"
    ...
    Context "Behavior" {
        It "Forwards to the canonical detection script and preserves its documented exit code" {
            Mock Write-Host { }
            Mock Write-Warning { }
            Mock Invoke-CanonicalScript { 1 }   # canonical: 1 = non-compliant/update available
            Main | Should -Be 1
            Should -Invoke Invoke-CanonicalScript -Times 1 -Exactly -ParameterFilter {
                $Path -like '*Test-WingetLenovoSystemUpdate.ps1'
            }
        }

        It "Returns 0 when the canonical script reports compliant (repeatable forward)" {
            Mock Write-Host { }
            Mock Write-Warning { }
