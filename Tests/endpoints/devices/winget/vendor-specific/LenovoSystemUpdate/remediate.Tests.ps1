...
Describe "remediate.ps1" {
    ...
    Context "Behavior" {
        It "Forwards to the canonical remediation script and preserves its documented exit code" {
            Mock Write-Host { }
            Mock Write-Warning { }
            Mock Invoke-CanonicalScript { 1 }   # canonical: 1 = remediation failed / needs retry
            Main | Should -Be 1
            Should -Invoke Invoke-CanonicalScript -Times 1 -Exactly -ParameterFilter {
                $Path -like '*Invoke-WingetLenovoSystemUpdate.ps1'
            }
        }
