...
Describe 'endpoints/devices/autopatch/V2/AP_Remediation' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../../scripts/endpoints/devices/autopatch/V2/AP_Remediation.ps1'
    ...
    Context 'Help & Metadata' {
        ...
        It 'Records the actual disk file name in .NOTES' {
            $rawText = Get-Content -Path $scriptPath -Raw
            $rawText | Should -Match "(?m)^\s*File Name\s*:\s*AP_Remediation.ps1\s*$"
        }
