...
Describe 'endpoints/devices/autopatch/V2/AP_Detection' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../../scripts/endpoints/devices/autopatch/V2/AP_Detection.ps1'
    ...
    Context 'Help & Metadata' {
        ...
        It 'Records the actual disk file name in .NOTES' {
            $rawText = Get-Content -Path $scriptPath -Raw
            $rawText | Should -Match "(?m)^\s*File Name\s*:\s*AP_Detection.ps1\s*$"
        }
    ...
    Context 'Behavior' {
        It 'Returns 0 and stops silently when no drift is found' {
            Mock New-Item { }
            Mock Start-Transcript { }
            Mock Stop-Transcript { }
            Mock Write-Verbose { }
        ...
        It 'Returns 1 when a managed policy value is present (drift)' {
            Mock New-Item { }
            Mock Start-Transcript { }
            Mock Stop-Transcript { }
            Mock Write-Verbose { }
            Mock Get-Item { [pscustomobject]@{ Property = @('DoNotConnectToWindowsUpdateInternetLocations') } }
            Main | Should -Be 1
        }

        It 'Returns 1 and writes [-] prefixed output when detection fails' {
            Mock New-Item { }
            Mock Start-Transcript { }
            Mock Stop-Transcript { }
            Mock Write-Verbose { }
            Mock Get-Item { throw 'registry unavailable' }
            $out = Main *>&1
