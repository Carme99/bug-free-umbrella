...
Describe 'endpoints/drivers/UnblockAMDDriver' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/devices/drivers/UnblockAMDDriver.ps1'
    ...
    Context 'Help & Metadata' {
        ...
        It 'Records the actual disk file name in .NOTES' {
            $rawText = Get-Content -Path $scriptPath -Raw
            $rawText | Should -Match "(?m)^\s*File Name\s*:\s*UnblockAMDDriver.ps1\s*$"
        }
    ...
    Context 'Behavior' {
        It 'Returns 0 and removes nothing when the policy key does not exist' {
        ...
        It 'Removes only the AMD entry and clears the retroactive flag' {
            function Set-ItemProperty { [CmdletBinding()] param([string]$Path, [string]$Name, [object]$Value, [string]$Type, [switch]$Force)}
            Mock Test-Path { $true }
            Mock Get-ItemProperty -ParameterFilter { $Name -eq 'DenyDeviceIDs' } { [pscustomobject]@{ DenyDeviceIDs = @('OTHERHWID', 'PCIVEN_1002&DEV_1681') } }
        ...
        It 'Removes the whole value when the list contained only the AMD entry' {
            function Set-ItemProperty { [CmdletBinding()] param([string]$Path, [string]$Name, [object]$Value, [string]$Type, [switch]$Force)}
            Mock Test-Path { $true }
            Mock Get-ItemProperty -ParameterFilter { $Name -eq 'DenyDeviceIDs' } { [pscustomobject]@{ DenyDeviceIDs = @('PCIVEN_1002&DEV_1681') } }
            Mock Get-ItemProperty { [pscustomobject]@{ DenyDeviceIDsRetroactive = 1 } }
            Mock Remove-ItemProperty { }
            Mock Set-ItemProperty { }
            $out = Main *>&1
            ($out | Where-Object { $_ -is [int] }) | Should -Be 0
        ...
        It 'Returns 1 and writes [-] prefixed output when the registry change fails' {
            function Set-ItemProperty { [CmdletBinding()] param([string]$Path, [string]$Name, [object]$Value, [string]$Type, [switch]$Force)}
            Mock Test-Path { $true }
            Mock Get-ItemProperty -ParameterFilter { $Name -eq 'DenyDeviceIDs' } { [pscustomobject]@{ DenyDeviceIDs = @('OTHERHWID', 'PCIVEN_1002&DEV_1681') } }
