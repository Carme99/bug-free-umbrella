...
Describe 'endpoints/drivers/BlockAMDDriver' {
    BeforeAll {
        $scriptPath = Join-Path $PSScriptRoot '../../../../scripts/endpoints/devices/drivers/BlockAMDDriver.ps1'
    ...
    Context 'Help & Metadata' {
        ...
        It 'Records the actual disk file name in .NOTES' {
            $rawText = Get-Content -Path $scriptPath -Raw
            $rawText | Should -Match "(?m)^\s*File Name\s*:\s*BlockAMDDriver.ps1\s*$"
        }
    ...
    Context 'Behavior' {
        It 'Applies the deny list and retroactive flag and returns 0' {
