...
cd "C:\bug-free-umbrella\scripts\endpoints\intune\reporting"
...
```powershell
# Still in scripts\endpoints\intune\reporting directory
...
```powershell
# Find-StaleDevices.ps1 lives in the maintenance folder
cd ..\maintenance
...
cd "C:\bug-free-umbrella\scripts\infrastructure\windows\system"
...
```powershell
# Disk scripts live in the storage folder
cd ..\storage
...
```powershell
# AD scripts live in the active-directory folder
cd ..\active-directory
...
```powershell
# Firewall and network scripts live in the network folder
cd ..\network
...
```powershell
# Security scripts live in the security folder
cd ..\security
...
$action = New-ScheduledTaskAction -Execute 'PowerShell.exe' `
    -Argument '-ExecutionPolicy Bypass -File "C:\bug-free-umbrella\scripts\infrastructure\windows\system\Check-SystemIntegrity.ps1" -GenerateReport'
...
```powershell
# Find-StaleDevices.ps1 lives in the maintenance folder
cd "C:\bug-free-umbrella\scripts\endpoints\intune\maintenance"
...
```powershell
# Navigate to the BitLocker key backup remediation
cd "C:\bug-free-umbrella\scripts\endpoints\devices\proactive-remediations\Fix-BitLockerNotEscrowedKeys"
```
...
From `scripts/endpoints/devices/proactive-remediations`:
...
```powershell
cd "C:\bug-free-umbrella\scripts\endpoints\devices\proactive-remediations\Fix-DiskSpace"
...
- [Script Catalog](Script-Catalog.md) - Complete reference
- [Support Guide](../SUPPORT.md) - Response times and support channels
- [Governance](../GOVERNANCE.md) - Project governance and solo maintainer model
