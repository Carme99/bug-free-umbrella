...
<#
.SYNOPSIS
    Test a Windows system against CIS Benchmark security controls.
.DESCRIPTION
    Tests the local system against CIS Microsoft Windows Server benchmark controls covering password
    policies, account lockout policies, and advanced audit policies. Security policy settings are
    exported with secedit.exe and audit settings with auditpol.exe; both native tools are invoked
    through thin wrapper functions so they can be mocked in automated tests.

    Requires elevation (administrator privileges) to export security policies; without elevation the
    export fails and the script exits 1. The optional HTML report is written under -OutputPath.

    Exit codes: 0 = every applicable control passed; 1 = one or more controls failed or errored, or
    the security policy export failed.
.PARAMETER Level
...
.PARAMETER ExportHTML
    Export results to an HTML report under -OutputPath.
.PARAMETER OutputPath
    Custom output path for the HTML report. Default: MyDocuments\Reports
.EXAMPLE
    PS C:\> .\Test-CISBenchmark.ps1

    Runs Level 1 CIS benchmark tests with console output.
.EXAMPLE
    PS C:\> .\Test-CISBenchmark.ps1 -Level 2 -ExportHTML

    Runs Level 2 tests and exports an HTML report to the Reports folder.
.NOTES
...
function Get-SecurityPolicy {
    ...
    try {
        ...
        throw "Failed to retrieve security policy: $($_.Exception.Message)"
    }
    finally {
...
# Helper to check audit policy ($auditOutput is resolved from the caller's scope)
...
function Main {
    ...
    try {
        ...
        if (-not (Test-AdministratorElevation)) {
            Write-Warning ("Administrator privileges are required to export security policies; " +
        ...
        if (-not $secPolicy) {
            throw "Failed to retrieve security policies."
        }
        ...
        if ($auditQuery.ExitCode -ne 0) {
            Write-Warning ("Failed to retrieve audit policies (auditpol.exe exited with code " +
                "$($auditQuery.ExitCode)); audit controls will be evaluated against no data.")
            $auditOutput = @()
        }
        else {
            $auditOutput = $auditQuery.Output
        ...
        if ($compliancePercent -ge 80) { $rateColor = "Green" }
        elseif ($compliancePercent -ge 60) { $rateColor = "Yellow" }
        else { $rateColor = "Red" }
        ...
        Write-Host "[*] Compliance Rate:     $compliancePercent%" -ForegroundColor $rateColor
        Write-Host "[*] ==================================================================`n" -ForegroundColor Cyan
        ...
        if ($ExportHTML) {
            $reportName = "CIS-Benchmark-Report_Level$($Level)_${RunTimestamp}_${RunId}.html"
            $htmlFile = Join-Path $resolvedOutputPath $reportName
...
<head>
    ...
    <style>
        ...
        .container { background: white; padding: 30px; border-radius: 8px; max-width: 1200px;
        ...
        .summary-grid { display: grid; gap: 15px; margin-top: 15px; }
...
"@
        ...
        # Exit code contract: 0 = compliant, 1 = at least one control failed/errored
        if ($failed -gt 0) {
            return 1
        }
        return 0
