...
.DESCRIPTION
    ...
    and various exploit protection mechanisms.
...
.EXAMPLE
    ...
    Tests features and generates reports
...
try {
    ...
    $OSVolume = $BitLockerVolumes | Where-Object { $_.VolumeType -eq 'OperatingSystem' } | Select-Object -First 1
...
try {
    ...
    if (-not $BootMode) {
        # Alternative method
        $BootMode = (Get-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control" -Name "PEFirmwareType" -ErrorAction SilentlyContinue).PEFirmwareType
    }
...
$EnabledFeatures = ($Results | Where-Object { $_.Status -eq 'Enabled' }).Count
$DisabledFeatures = ($Results | Where-Object { $_.Status -in @('Disabled', 'Not Available', 'Failed') }).Count
...
if ($ShowRecommendations -or $DisabledFeatures -gt 0) {
    ...
    $DisabledItems = $Results | Where-Object { $_.Status -in @('Disabled', 'Not Available') -and $_.Recommendation }
...
<body>
    ...
    <div class="summary">
        ...
        <div class="summary-item"><strong>Disabled:</strong> <span style="color: #e74c3c;">$DisabledFeatures</span></div>
    </div>

    <div class="score $(if ($SecurityScore -ge 80) { 'good' } elseif ($SecurityScore -ge 60) { 'medium' } else { 'poor' })">
