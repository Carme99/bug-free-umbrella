# Proactive Remediation Detection Script for Adobe Acrobat (64-bit) via Winget
$name = 'Adobe Acrobat'
...
try {
    ...
    if ($lines -match '\bVersion\s+Available\b') {
        Write-Output "$name (64-bit) update available."
        Exit 1
    } elseif ($lines -match "No installed package found matching input criteria.") {
        Write-Output "$name (64-bit) is not installed."
        Exit 0
    } else {
        Write-Output "$name (64-bit) is already up to date."
        Exit 0
    }
} catch {
