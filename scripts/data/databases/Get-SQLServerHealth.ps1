<#
.SYNOPSIS
    Check SQL Server instance health, databases, backups, logs, jobs, and performance.
...
.EXAMPLE
    PS C:\> .\Get-SQLServerHealth.ps1
    Checks health of local SQL Server instance.

.EXAMPLE
    PS C:\> .\Get-SQLServerHealth.ps1 -ServerInstance "SQLSERVER01" -CheckBackups -ExportHTML
    Comprehensive health check with backup validation.

.EXAMPLE
    PS C:\> .\Get-SQLServerHealth.ps1 -Database "ProductionDB" -IncludePerformance
...
"@
    ...
    if ($databases) {
        foreach ($db in $databases) {
            ...
            $dbColor = if ($status -eq "Pass") { "Green" } else { "Red" }
            Write-Host "[$status] $($db.DatabaseName): $($db.Status) ($($db.SizeMB) MB)" -ForegroundColor $dbColor
...
"@
    ...
    if ([string]::IsNullOrWhiteSpace($ReportDir) -or
        $ReportDir -match '(^|[\\/])\.\.([\\/]|$)' -or
        $ReportDir -match '^(\\\\|//)') {
        Write-Error ("Unsafe report directory: $ReportDir. Report directory must be a local " +
    ...
    if ($ExportHTML) {
        ...
        $scoreColorHex = if ($healthScore -ge 80) { '#27ae60' }
    ...
    <head>
        ...
        <style>
            ...
            .score { font-size: 24px; font-weight: bold; color: $scoreColorHex; }
