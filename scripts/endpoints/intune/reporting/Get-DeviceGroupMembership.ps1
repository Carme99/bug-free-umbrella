...
try {
    ...
    if ($DeviceName) {
        ...
        $azureAdDevices = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/devices?`$filter=displayName eq '$DeviceName'"
        ...
        $memberOf = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/devices/$($device.id)/memberOf"

        if ($memberOf.value) {
            foreach ($group in $memberOf.value) {
                # Get group details
                $groupDetails = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/groups/$($group.id)"
    ...
    elseif ($GroupName) {
        ...
        # Find group
        $groups = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/groups?`$filter=displayName eq '$GroupName'"
        ...
        $members = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/groups/$($group.id)/members"
    ...
    elseif ($ShowAllMappings) {
        ...
        $allGroups = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/groups"
        ...
        foreach ($group in $groups) {
            ...
            # Get members
            try {
                $members = Invoke-MgGraphRequest -Uri "https://graph.microsoft.com/v1.0/groups/$($group.id)/members?`$select=id,displayName,deviceId" -ErrorAction SilentlyContinue
    ...
    if ($ShowAllMappings -or $DeviceName) {
        ...
        if ($ShowAllMappings) {
            $topGroups = $report | Group-Object -Property GroupName | Sort-Object Count -Descending | Select-Object -First 10
    ...
    if ($ExportFormat -eq 'HTML' -or $ExportFormat -eq 'Both') {
        ...
        Export-IntuneReportToHTML -Data $report -Title "Device Group Membership Report" -FilePath $htmlPath
    }

    if ($ExportFormat -eq 'CSV' -or $ExportFormat -eq 'Both') {
        $csvPath = Join-Path $outputPath "$reportTitle.csv"
        Export-IntuneReportToCSV -Data $report -Title $reportTitle -FilePath $csvPath
    }
