<#
.SYNOPSIS
    [DEPRECATED] Forward to the canonical remediation script Invoke-WingetTeamViewerFullForceClose.ps1.

.DESCRIPTION
    Deprecated compatibility shim retained so existing Intune Proactive Remediations assignments keep
    working. Invokes the canonical script at scripts/endpoints/remediation/winget/remote-access/TeamViewerFull/Invoke-WingetTeamViewerFullForceClose.ps1 and preserves its exit code. Update
...
.NOTES
    File Name  : remediate_Force_Close.ps1
...
function Main {
    try {
        ...
        $canonicalFile = 'remediation/winget/remote-access/TeamViewerFull/Invoke-WingetTeamViewerFullForceClose.ps1'
