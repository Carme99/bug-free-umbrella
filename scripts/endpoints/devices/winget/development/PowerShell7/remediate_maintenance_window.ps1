<#
.SYNOPSIS
    [DEPRECATED] Forward to the canonical remediation script Invoke-WingetPowerShell7MaintenanceWindow.ps1.

.DESCRIPTION
    Deprecated compatibility shim retained so existing Intune Proactive Remediations assignments keep
    working. Invokes the canonical remediation script named in the synopsis from the shared
    remediation tree and preserves its exit code. Update Intune assignments to the canonical
    path; this shim will be removed in version 6.0.0.
...
.EXAMPLE
    PS C:\> .\remediate_maintenance_window.ps1
...
.EXAMPLE
    PS C:\> pwsh -NoProfile -File .\remediate_maintenance_window.ps1

    Runs the shim in a clean process; forwarding behavior is unchanged.

.NOTES
    File Name  : remediate_maintenance_window.ps1
...
function Main {
    try {
        ...
        $canonicalFile = 'remediation/winget/development/PowerShell7/Invoke-WingetPowerShell7MaintenanceWindow.ps1'
