<#
.SYNOPSIS
    [DEPRECATED] Forward to the canonical remediation script Invoke-WingetCpp2008Redist.ps1.

.DESCRIPTION
    Deprecated compatibility shim retained so existing Intune Proactive Remediations assignments keep
    working. Invokes the canonical script at scripts/endpoints/remediation/winget/runtimes/Cpp2008Redist/Invoke-WingetCpp2008Redist.ps1 and preserves its exit code. Update
...
function Main {
    try {
        ...
        $canonicalFile = 'remediation/winget/runtimes/Cpp2008Redist/Invoke-WingetCpp2008Redist.ps1'
