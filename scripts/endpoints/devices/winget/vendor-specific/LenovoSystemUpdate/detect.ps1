<#
.SYNOPSIS
    [DEPRECATED] Forward to the canonical detection script Test-WingetLenovoSystemUpdate.ps1.
...
.DESCRIPTION
    ...
    scripts/endpoints/remediation/winget/vendor-specific/LenovoSystemUpdate/Test-WingetLenovoSystemUpdate.ps1
    and preserves its exit code. Update Intune assignments to the canonical path; this shim will be
    removed in version 6.0.0.
    Exit codes: passes through the canonical script's exit code (Intune convention: 0 =
    compliant/up to date or absent, 1 = non-compliant/update available).
...
function Main {
    try {
        Write-Host '[*] Forwarding to canonical detection script...' -ForegroundColor Cyan

        $canonicalFile = 'remediation/winget/vendor-specific/LenovoSystemUpdate/Test-WingetLenovoSystemUpdate.ps1'
