...
.DESCRIPTION
    Removes the NoAutoUpdate value from HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU so the device
    returns to default Windows Update behaviour.
    ...
    failure.
    DisableWindowsUpdateAccess itself ("Remove access to use all Windows Update features") is unsupported on
    Windows 10+ / Server 2016+; this pair manages the supported WUaaS control NoAutoUpdate instead. See
...
$script:RegValueName = 'NoAutoUpdate'
