...
.DESCRIPTION
    Applies the DeviceInstallation policy that blocks the AMD Radeon hardware ID (PCIVEN_1002&DEV_1681) by
    merging it
    into the DenyDeviceIDs multi-string value under
    HKLM\SOFTWARE\Policies\Microsoft\Windows\DeviceInstall\Restrictions
    and setting the DenyDeviceIDsRetroactive flag so already-installed devices are blocked too.
    Idempotent check-then-act: an already-converged system is detected and left unchanged. Registry mutations
    are gated
    behind -WhatIf/-Confirm. Exits 0 on success (including already-converged) and 1 on failure.
...
.EXAMPLE
    PS C:\> .\BlockAMDDriver.ps1

    Applies the driver block; exits 0.

.EXAMPLE
    PS C:\> .\BlockAMDDriver.ps1 -WhatIf
