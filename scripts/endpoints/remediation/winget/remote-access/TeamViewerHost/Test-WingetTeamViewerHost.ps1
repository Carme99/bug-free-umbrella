<#
.SYNOPSIS
    Detects pending TeamViewer Host updates (winget id TeamViewer.TeamViewer.Host) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the TeamViewer Host update pair. Checks whether a TeamViewer Host update is
    available and exits non-compliant so remediation can be triggered. Prefers the Microsoft.WinGet.Client
    ...
    Exit codes:
    - 0: compliant - TeamViewer Host is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetTeamViewerHost.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetTeamViewerHost.ps1
    Runs the detection check and exits 0 when TeamViewer Host is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetTeamViewerHost.ps1'
...
function Main {
    try {
        ...
        $namePattern = "^(TeamViewer\.TeamViewer\.Host)\s+(.+?)\s+\d"
        $name = if ($packageInfo -match $namePattern) { $Matches[2].Trim() } else { $ID }
