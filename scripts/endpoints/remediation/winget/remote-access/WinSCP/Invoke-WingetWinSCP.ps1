<#
.SYNOPSIS
    Updates WinSCP (winget id WinSCP.WinSCP) silently, deferring while the app runs.

.DESCRIPTION
    Remediation half of the WinSCP update pair. Checks whether a WinSCP update is available and
    installs it silently with winget. When the WinSCP process is running the update is deferred
    (exit 1) so Intune can retry later instead of interrupting an active transfer session.
    Prefers the
    ...
    - 1: failure - WinSCP is running (deferred), verification failed after update, or an error occurred.
    Re-running on a converged system exits 0 without changes (idempotent).

.NOTES
    File Name: Invoke-WingetWinSCP.ps1
...
.EXAMPLE
    ...
    Updates WinSCP silently; exits 1 without changes when WinSCP is running.
...
function Main {
    ...
    try {
        ...
        $name = if ($packageInfo -match "^($ID)\s+(.+?)\s+\d") { $Matches[2].Trim() } else { 'WinSCP' }
