<#
.SYNOPSIS
    Detects pending updates for WinSCP (winget id WinSCP.WinSCP) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the WinSCP update pair. Checks whether a WinSCP update is available and
    ...
    Exit codes:
    - 0: compliant - WinSCP is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetWinSCP.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetWinSCP.ps1
    Runs the detection check and exits 0 when WinSCP is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetWinSCP.ps1'
...
function Main {
    try {
        ...
        $name = if ($packageInfo -match "^(WinSCP\.WinSCP)\s+(.+?)\s+\d") { $Matches[2].Trim() } else { $ID }
