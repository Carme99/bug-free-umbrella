<#
.SYNOPSIS
    Updates Zoom (winget id Zoom.Zoom) silently for Intune Proactive Remediations.

.DESCRIPTION
    Remediation half of the Zoom update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the Zoom process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetZoom.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetZoom.ps1
    Updates Zoom silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetZoom.ps1 -WhatIf
