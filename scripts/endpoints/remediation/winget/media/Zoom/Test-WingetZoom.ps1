<#
.SYNOPSIS
    Detects pending updates for Zoom (winget id Zoom.Zoom) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Zoom update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - Zoom is up to date, not installed, or the check could not run (network down,
...
.EXAMPLE
    PS C:\> .\Test-WingetZoom.ps1
    Runs the detection check and exits 0 when Zoom is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetZoom.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetZoom.ps1
        ...
        if (Get-Module -ListAvailable -Name Microsoft.WinGet.Client) {
            ...
            if (Get-Command Get-WinGetPackage -ErrorAction SilentlyContinue) {
                ...
                if ($package.IsUpdateAvailable) {
                    ...
                    $msg = "[!] Application update available for $($package.Name). "
                    $msg += "Current: $verInstalled, Available: $verAvailable"
                ...
                $msg = "[+] $($package.Name) is already up to date (version $($package.InstalledVersion))."
