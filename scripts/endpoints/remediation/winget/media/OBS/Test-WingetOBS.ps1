<#
.SYNOPSIS
    Detects pending updates for OBS Studio (winget id OBSProject.OBSStudio) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the OBS Studio update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - OBS Studio is up to date, not installed, or the check could not run (network
      down or transient winget failure); failures deliberately exit 0 so remediation is never
      falsely triggered.
    - 1: non-compliant - an application update is available.

.EXAMPLE
    PS C:\> .\Test-WingetOBS.ps1
    Runs the detection check and exits 0 when OBS Studio is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetOBS.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetOBS.ps1
