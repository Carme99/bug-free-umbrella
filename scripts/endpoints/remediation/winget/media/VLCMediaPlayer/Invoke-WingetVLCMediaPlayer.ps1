<#
.SYNOPSIS
    Updates VLC Media Player (winget id VideoLAN.VLC) silently for Intune Proactive Remediations.

.DESCRIPTION
    Standard remediation for VLC Media Player - waits for the player to close before updating so
    user playback is never interrupted. Prefers the Microsoft.WinGet.Client PowerShell module
    ...
    If the vlc process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetVLCMediaPlayer.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetVLCMediaPlayer.ps1
    Updates VLC Media Player silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetVLCMediaPlayer.ps1 -WhatIf
