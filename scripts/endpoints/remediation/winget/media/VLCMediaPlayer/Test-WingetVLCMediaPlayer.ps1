<#
.SYNOPSIS
    Detects pending updates for VLC Media Player (winget id VideoLAN.VLC) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the VLC Media Player update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - VLC Media Player is up to date, not installed, its version could not be parsed,
      or the check could not run (network down or transient winget failure); failures deliberately
      exit 0 so remediation is never falsely triggered.
    - 1: non-compliant - an application update is available.

.EXAMPLE
    PS C:\> .\Test-WingetVLCMediaPlayer.ps1
    Runs the detection check and exits 0 when VLC is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetVLCMediaPlayer.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetVLCMediaPlayer.ps1
    ...
    try {
        ...
        if ($packageInfo -match '\d+(\.\d+)+') {
            ...
            return 0
                ...
                $outputMsg = "[!] $name appears to be installed but version info could not be determined; treating as `
            compliant."
        Write-Host $outputMsg -ForegroundColor Yellow
        return 0
    }
