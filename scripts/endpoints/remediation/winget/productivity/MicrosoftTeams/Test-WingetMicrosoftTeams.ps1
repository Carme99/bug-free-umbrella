<#
.SYNOPSIS
    Detects pending updates for Microsoft Teams (winget id Microsoft.Teams).

.DESCRIPTION
    Detection half of the Microsoft Teams update pair. Checks whether a Teams update is available
    and exits non-compliant so remediation can be triggered. Prefers the Microsoft.WinGet.Client
    ...
    Exit codes:
    - 0: compliant - Microsoft Teams is up to date, not installed, the device is offline, or the
      check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetMicrosoftTeams.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetMicrosoftTeams.ps1
    Runs the detection check and exits 0 when Teams is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetMicrosoftTeams.ps1'
