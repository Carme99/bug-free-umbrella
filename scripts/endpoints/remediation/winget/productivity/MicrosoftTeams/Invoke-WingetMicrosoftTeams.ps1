<#
.SYNOPSIS
    Force-closes Microsoft Teams and updates it silently (winget id Microsoft.Teams).

.DESCRIPTION
    Remediation half of the Microsoft Teams update pair. Checks whether a Teams update is available
    and, when the ms-teams process is running, optionally notifies the logged-on user before force
    closing the process so the silent install can proceed (WARNING: force closing may cause data
    loss). Prefers the Microsoft.WinGet.Client PowerShell module because the winget CLI is not
...
.NOTES
    File Name: Invoke-WingetMicrosoftTeams.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetMicrosoftTeams.ps1
    Force closes Teams if running, then updates it silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetMicrosoftTeams.ps1 -WhatIf
...
function Show-UserNotification {
    ...
    try {
        ...
        $notificationMessage += "Please save your work."
