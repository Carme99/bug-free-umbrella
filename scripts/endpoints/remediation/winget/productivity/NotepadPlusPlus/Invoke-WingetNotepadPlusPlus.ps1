<#
.SYNOPSIS
    Updates Notepad++ (winget id Notepad++.Notepad++) silently.

.DESCRIPTION
    Standard remediation for Notepad++ - defers the update while notepad++.exe is running so open
    documents are never lost. Prefers the Microsoft.WinGet.Client PowerShell module because the
    winget CLI is not supported in the SYSTEM context that Intune Proactive Remediations use; when
    the module is unavailable it falls back to the winget.exe CLI through the Invoke-WingetWithRetry
    wrapper (the only place a native executable is called).
    If the notepad++.exe process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.EXAMPLE
    ...
    Updates Notepad++ silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetNotepadPlusPlus.ps1 -WhatIf
...
function Main {
    ...
    try {
        ...
        $name = if ($packageInfo -match "^($([regex]::Escape($ID)))\s+(.+?)\s+\d") {
            $Matches[2].Trim()
        }
        else {
            "NotePad++"
        ...
        if ($packageInfo -match '\bVersion\s+Available\b') {
            ...
            # Check if NotePad++ is open - don't interrupt the user
            if ($process) {
