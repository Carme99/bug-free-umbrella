<#
.SYNOPSIS
    Updates Adobe Acrobat Reader 64-bit (winget id Adobe.Acrobat.Reader.64-bit) silently.

.DESCRIPTION
    Standard remediation for Adobe Acrobat Reader 64-bit - defers the update while AcroRd64 is
    ...
    If the AcroRd64 process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetAdobeReader64bit.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetAdobeReader64bit.ps1
    Updates Adobe Acrobat Reader 64-bit silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetAdobeReader64bit.ps1 -WhatIf
...
function Main {
    ...
    try {
        ...
        if ($packageInfo -match '\bVersion\s+Available\b') {
            ...
            # Check if Adobe Acrobat Reader 64-bit is open - don't interrupt the user
            if ($process) {
