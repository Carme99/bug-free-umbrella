<#
.SYNOPSIS
    Detects pending updates for Adobe Acrobat Reader 64-bit (winget id Adobe.Acrobat.Reader.64-bit).

.DESCRIPTION
    Detection half of the Adobe Acrobat Reader 64-bit update pair. Prefers the Microsoft.WinGet.Client
    ...
    Exit codes:
    - 0: compliant - Adobe Acrobat Reader 64-bit is up to date, not installed, its version could not
      be parsed, or the check could not run (network down or transient winget failure); failures
      deliberately exit 0 so remediation is never falsely triggered.
    - 1: non-compliant - an application update is available.

.EXAMPLE
    PS C:\> .\Test-WingetAdobeReader64bit.ps1
    Runs the detection check and exits 0 when Reader is current or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetAdobeReader64bit.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetAdobeReader64bit.ps1
