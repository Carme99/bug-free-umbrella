<#
.SYNOPSIS
    Detects pending updates for Adobe Acrobat Reader 32-bit (winget id Adobe.Acrobat.Reader.32-bit).

.DESCRIPTION
    Detection half of the Adobe Acrobat Reader 32-bit update pair. Checks whether an update is
    available and exits non-compliant so remediation can be triggered. Prefers the
    ...
    executable is called). A network connectivity check runs first; when offline the check reports
    compliant because no update query is possible, and any internal error also reports compliant so
    remediation is never falsely triggered.
    Exit codes:
    - 0: compliant - Adobe Acrobat Reader 32-bit is up to date, not installed, the device is
      offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetAdobeReader32bit.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetAdobeReader32bit.ps1
    Runs the detection check and exits 0 when Reader is current or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetAdobeReader32bit.ps1'
...
function Main {
    try {
        ...
        if ($packageInfo -match '\bVersion\s+Available\b') {
            ...
            $outputMsg = "Update available for $name | Installed: $verInstalled | Available: $verAvailable"
            Write-Log $outputMsg -Level Info
            $updateMsg = "[!] Application update available for $name. Current: $verInstalled, Available: $verAvailable"
            Write-Host $updateMsg -ForegroundColor Yellow
        ...
        $outputMsg = "ERROR: Failed to check $name for updates: $errMsg"
