<#
.SYNOPSIS
    Detects pending updates for Microsoft Remote Desktop (winget id Microsoft.RemoteDesktopClient).

.DESCRIPTION
    Detection half of the Microsoft Remote Desktop update pair. Prefers the Microsoft.WinGet.Client
    ...
    Exit codes:
    - 0: compliant - Microsoft Remote Desktop is up to date, not installed, its version could not be
      parsed, or the check could not run (network down or transient winget failure); failures
      deliberately exit 0 so remediation is never falsely triggered.
    - 1: non-compliant - an application update is available.

.EXAMPLE
    PS C:\> .\Test-WingetRemoteDesktop.ps1
    Runs the detection check and exits 0 when Remote Desktop is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetRemoteDesktop.ps1'
