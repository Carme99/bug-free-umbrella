<#
.SYNOPSIS
    Updates Microsoft Remote Desktop (winget id Microsoft.RemoteDesktopClient) silently.

.DESCRIPTION
    Standard remediation for Microsoft Remote Desktop - defers the update while the
    Microsoft.RemoteDesktop process is running so active sessions are never interrupted. Prefers the
    ...
    If the Microsoft.RemoteDesktop process is running, the update is deferred (exit 1) so Intune
    retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetRemoteDesktop.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetRemoteDesktop.ps1
    Updates Microsoft Remote Desktop silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetRemoteDesktop.ps1 -WhatIf
...
$AppProcess = 'Microsoft.RemoteDesktop'
$MaxRetries = 3
$VerifyWaitSeconds = 5
#endregion
...
function Main {
    ...
    try {
        ...
        if ($packageInfo -match '\bVersion\s+Available\b') {
            ...
            # Check if a Remote Desktop session is open - don't interrupt the user
            if ($process) {
