<#
.SYNOPSIS
    Detects pending 7-Zip updates (winget id 7zip.7zip) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the 7-Zip update pair. Checks whether an update is available and
    ...
    Exit codes:
    - 0: compliant - 7-Zip is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetSevenZip.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetSevenZip.ps1
    Runs the check; exits 0 when 7-Zip is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetSevenZip.ps1'
