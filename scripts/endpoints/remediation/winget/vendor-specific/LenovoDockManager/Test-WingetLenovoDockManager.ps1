<#
.SYNOPSIS
    Detects pending Lenovo Dock Manager updates (winget id Lenovo.DockManager) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Lenovo Dock Manager update pair. Checks whether an update is available and
    ...
    Exit codes:
    - 0: compliant - Lenovo Dock Manager is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetLenovoDockManager.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetLenovoDockManager.ps1
    Runs the check; exits 0 when Lenovo Dock Manager is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetLenovoDockManager.ps1'
