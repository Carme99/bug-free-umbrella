<#
.SYNOPSIS
    Silently updates Lenovo Dock Manager via winget (skips while running) for Intune Proactive Remediations.

.DESCRIPTION
    Remediation half of the Lenovo Dock Manager update pair. Checks whether an update is available and installs it
    silently through the Microsoft.WinGet.Client module, falling back to the winget.exe CLI through the
    Invoke-WingetWithRetry wrapper when the module is unavailable. When the LenovoDockMgr process is running the
...
.NOTES
    File Name: Invoke-WingetLenovoDockManager.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetLenovoDockManager.ps1
    Updates Lenovo Dock Manager silently unless LenovoDockMgr runs; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetLenovoDockManager.ps1 -WhatIf
...
function Main {
    ...
    try {
        ...
        $name = if ($nameMatch) { $nameMatch.Matches[0].Groups[2].Value.Trim() } else { "Lenovo Dock Manager" }
