<#
.SYNOPSIS
    Detects pending Lenovo System Update updates (winget id Lenovo.SystemUpdate) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Lenovo System Update update pair. Checks whether an update is available and
    exits non-compliant so remediation can be triggered. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Invoke-WingetWithRetry wrapper (the only place a native executable is called). A network connectivity
    check runs first; when offline the check reports compliant because no update query is possible, and
    any internal error also reports compliant so remediation is never falsely triggered.
    Exit codes:
    - 0: compliant - Lenovo System Update is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetLenovoSystemUpdate.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetLenovoSystemUpdate.ps1
    Runs the check; exits 0 when Lenovo System Update is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetLenovoSystemUpdate.ps1'
...
function Main {
    try {
        ...
        if (Get-Module -ListAvailable -Name Microsoft.WinGet.Client) {
            ...
            if (Get-Command Get-WinGetPackage -ErrorAction SilentlyContinue) {
                ...
                if ($package.IsUpdateAvailable) {
                    ...
                    $outputMsg = "Update available for $($package.Name) ($verInstalled -> $verAvailable)"
                    Write-Log $outputMsg -Level Info
                    Write-Host "[!] Update available for $($package.Name): $verInstalled -> $verAvailable" `
                    ...
                    Write-Log $outputMsg -Level Info
                    Write-Host "[+] $($package.Name) is already up to date (version $versionInstalled)." `
        ...
        if ($packageInfo -match '\bVersion\s+Available\b') {
            ...
            Write-Log $outputMsg -Level Info
            Write-Host "[!] Update available for $($name): $verInstalled -> $verAvailable" -ForegroundColor Yellow
            return 1  # Trigger remediation
        }
        ...
        Write-Host "[-] Failed to check $ID for updates: $($_.Exception.Message)" -ForegroundColor Red
        return 0
    }
    finally {
