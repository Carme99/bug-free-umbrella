<#
.SYNOPSIS
    Detects pending VCRedist updates for winget package Microsoft.VCRedist.2013.x64 for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Visual C++ Redistributable update pair. Checks whether an update is available for
    winget package Microsoft.VCRedist.2013.x64 and exits non-compliant so remediation can be triggered. Prefers the
...
.NOTES
    File Name: Test-WingetCpp2013RedistX64.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetCpp2013RedistX64.ps1
    Runs the detection check; exits 0 when up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetCpp2013RedistX64.ps1'
