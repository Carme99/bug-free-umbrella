<#
.SYNOPSIS
    Updates the C++ 2008 Redistributable silently, skipping while its process runs.

.DESCRIPTION
    Remediation half of the C++ 2008 Redistributable update pair. Checks whether an update is
    available and installs it silently; this variant never terminates processes - when the
    Microsoft.VCRedist.2008.x86 process is running the update is skipped so Intune can retry on a
...
.NOTES
    File Name: Invoke-WingetCpp2008Redist.ps1
...
$AppProcess = 'Microsoft.VCRedist.2008.x86'
$MaxRetries = 3
$VerifyWaitSeconds = 5
...
function Main {
    ...
    try {
        ...
        $pattern = "^($ID)\s+(.+?)\s+\d"
        $name = if ($packageInfo | Select-String -Pattern $pattern) { $Matches[2].Trim() } else { 'C++ 2008 Redist' }
