<#
.SYNOPSIS
    Detects pending Edge WebView2 updates (winget id Microsoft.EdgeWebView2Runtime) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Edge WebView2 update pair. Checks whether an update is available and
    ...
    Exit codes:
    - 0: compliant - Edge WebView2 is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetEdgeWebView2.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetEdgeWebView2.ps1
    Runs the check; exits 0 when Edge WebView2 is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetEdgeWebView2.ps1'
