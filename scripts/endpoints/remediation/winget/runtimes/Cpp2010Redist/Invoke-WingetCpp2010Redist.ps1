<#
.SYNOPSIS
    Updates the C++ 2010 x86 Redistributable silently, skipping while its process runs.

.DESCRIPTION
    Remediation half of the C++ 2010 x86 Redistributable update pair. Checks whether an update is
    available and installs it silently; this variant never terminates processes - when the
    Microsoft.VCRedist.2010.x86 process is running the update is skipped so Intune can retry on a
...
.NOTES
    File Name: Invoke-WingetCpp2010Redist.ps1
