<#
.SYNOPSIS
    Detects pending updates for Discord (winget id Discord.Discord) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Discord update pair. Checks whether a Discord update is available and
    ...
    Exit codes:
    - 0: compliant - Discord is up to date, not installed, the device is offline, or the check failed.
...
.EXAMPLE
    ...
    Runs the detection check and exits 0 when Discord is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetDiscord.ps1'
