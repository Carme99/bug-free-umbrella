<#
.SYNOPSIS
    Detects pending updates for Slack (winget id SlackTechnologies.Slack) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Slack update pair. Checks whether a Slack update is available and
    ...
    Exit codes:
    - 0: compliant - Slack is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-WingetSlack.ps1
...
.EXAMPLE
    PS C:\> .\Test-WingetSlack.ps1
    Runs the detection check and exits 0 when Slack is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-WingetSlack.ps1'
        ...
        if (Get-Module -ListAvailable -Name Microsoft.WinGet.Client) {
            ...
            if (Get-Command Get-WinGetPackage -ErrorAction SilentlyContinue) {
                ...
                if ($package.IsUpdateAvailable) {
                    ...
                    Write-Log $outputMsg -Level Info
                                        $updateMsg = "[!] Application update available for $name. Current: `
                        $verInstalled, Available: $verAvailable"
