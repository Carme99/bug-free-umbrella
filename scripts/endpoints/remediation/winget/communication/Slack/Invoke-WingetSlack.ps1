<#
.SYNOPSIS
    Force-closes Slack and updates it silently (winget id Slack.Slack) for Intune Proactive Remediations.

.DESCRIPTION
    Remediation half of the Slack update pair. Checks whether a Slack update is available and,
...
.NOTES
    File Name: Invoke-WingetSlack.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetSlack.ps1
    Force closes Slack if running, then updates it silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetSlack.ps1 -WhatIf
