<#
.SYNOPSIS
Updates SQL Server Management Studio (winget id Microsoft.SQLServerManagementStudio) silently for Intune Proactive
    Remediations.
.DESCRIPTION
        Remediation half of the SQL Server Management Studio update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the SQL Server Management Studio process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.EXAMPLE
    PS C:\> .\Invoke-WingetSSMS.ps1
    Updates SQL Server Management Studio silently; exits 0 on success or when already up to date.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Invoke-WingetSSMS.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.
.NOTES
    File Name: Invoke-WingetSSMS.ps1
...
function Main {
    ...
    try {
        ...
        $name = if ($packageInfo -match "^(Microsoft.SQLServerManagementStudio)\s+(.+?)\s+\d") {
            $Matches[2].Trim()
        }
        else {
