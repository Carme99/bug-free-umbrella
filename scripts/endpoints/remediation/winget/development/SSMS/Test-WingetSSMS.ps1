<#
.SYNOPSIS
Detects pending updates for SQL Server Management Studio (winget id Microsoft.SQLServerManagementStudio) for
    Intune Proactive Remediations.
.DESCRIPTION
        Detection half of the SQL Server Management Studio update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
- 0: compliant - SQL Server Management Studio is up to date, not installed, or the check could not run (network
    down,
...
.EXAMPLE
    PS C:\> .\Test-WingetSSMS.ps1
Runs the detection check and exits 0 when SQL Server Management Studio is up to date or absent, 1 when an update
    is available.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetSSMS.ps1'
