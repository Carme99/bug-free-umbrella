<#
.SYNOPSIS
Detects pending updates for Visual Studio Professional 2022 (winget id Microsoft.VisualStudio.2022.Professional)
    for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Visual Studio Professional 2022 update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
- 0: compliant - Visual Studio Professional 2022 is up to date, not installed, or the check could not run (network
    down,
...
.EXAMPLE
    PS C:\> .\Test-WingetVisualStudioPro2022.ps1
Runs the detection check and exits 0 when Visual Studio Professional 2022 is up to date or absent, 1 when an
    update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetVisualStudioPro2022.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetVisualStudioPro2022.ps1
