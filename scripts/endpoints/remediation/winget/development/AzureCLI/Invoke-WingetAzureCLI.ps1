<#
.SYNOPSIS
    Updates Azure CLI (winget id Microsoft.AzureCLI) silently for Intune Proactive Remediations.
.DESCRIPTION
        Remediation half of the Azure CLI update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the Azure CLI process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.EXAMPLE
    PS C:\> .\Invoke-WingetAzureCLI.ps1
    Updates Azure CLI silently; exits 0 on success or when already up to date.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Invoke-WingetAzureCLI.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.
.NOTES
    File Name: Invoke-WingetAzureCLI.ps1
