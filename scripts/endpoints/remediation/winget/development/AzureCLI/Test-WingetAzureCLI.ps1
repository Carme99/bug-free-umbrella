<#
.SYNOPSIS
    Detects pending updates for Azure CLI (winget id Microsoft.AzureCLI) for Intune Proactive Remediations.
.DESCRIPTION
        Detection half of the Azure CLI update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - Azure CLI is up to date, not installed, or the check could not run (network down,
...
.EXAMPLE
    PS C:\> .\Test-WingetAzureCLI.ps1
    Runs the detection check and exits 0 when Azure CLI is up to date or absent, 1 when an update is available.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetAzureCLI.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.
.NOTES
    File Name: Test-WingetAzureCLI.ps1
...
function Main {
    try {
        ...
        $name = if ($packageInfo -match "^(Microsoft.AzureCLI)\s+(.+?)\s+\d") {
            $Matches[2].Trim()
        }
        else {
            "Azure CLI"
