<#
.SYNOPSIS
    Detects pending updates for PowerShell 7 (winget id Microsoft.PowerShell) for Intune Proactive Remediations.
.DESCRIPTION
        Detection half of the PowerShell 7 update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - PowerShell 7 is up to date, not installed, or the check could not run (network down,
    ...
    A maintenance-window remediation variant (Invoke-WingetPowerShell7MaintenanceWindow.ps1)
    ships alongside this detector for production environments.
.EXAMPLE
    PS C:\> .\Test-WingetPowerShell7.ps1
    Runs the detection check and exits 0 when PowerShell 7 is up to date or absent, 1 when an update is available.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetPowerShell7.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.
.NOTES
    File Name: Test-WingetPowerShell7.ps1
