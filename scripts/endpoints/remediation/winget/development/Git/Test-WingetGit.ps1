<#
.SYNOPSIS
    Detects pending updates for Git (winget id Git.Git) for Intune Proactive Remediations.
.DESCRIPTION
        Detection half of the Git update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - Git is up to date, not installed, or the check could not run (network down,
...
.EXAMPLE
    PS C:\> .\Test-WingetGit.ps1
    Runs the detection check and exits 0 when Git is up to date or absent, 1 when an update is available.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetGit.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.
.NOTES
    File Name: Test-WingetGit.ps1
