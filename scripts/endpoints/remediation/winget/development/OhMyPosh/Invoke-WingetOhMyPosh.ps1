<#
.SYNOPSIS
    Updates Oh My Posh (winget id JanDeDobbeleer.OhMyPosh) silently for Intune Proactive Remediations.
.DESCRIPTION
        Remediation half of the Oh My Posh update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the Oh My Posh process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.EXAMPLE
    PS C:\> .\Invoke-WingetOhMyPosh.ps1
    Updates Oh My Posh silently; exits 0 on success or when already up to date.
.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Invoke-WingetOhMyPosh.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.
.NOTES
    File Name: Invoke-WingetOhMyPosh.ps1
