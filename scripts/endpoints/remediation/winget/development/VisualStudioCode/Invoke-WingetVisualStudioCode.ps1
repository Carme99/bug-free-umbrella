<#
.SYNOPSIS
    Updates Visual Studio Code (winget id Microsoft.VisualStudioCode) silently for Intune Proactive Remediations.

.DESCRIPTION
    Remediation half of the Visual Studio Code update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the Visual Studio Code process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetVisualStudioCode.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetVisualStudioCode.ps1
    Updates Visual Studio Code silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetVisualStudioCode.ps1 -WhatIf
