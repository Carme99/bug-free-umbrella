<#
.SYNOPSIS
Detects pending updates for Visual Studio Code (winget id Microsoft.VisualStudioCode) for Intune Proactive
    Remediations.

.DESCRIPTION
    Detection half of the Visual Studio Code update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - Visual Studio Code is up to date, not installed, or the check could not run (network down,
...
.EXAMPLE
    PS C:\> .\Test-WingetVisualStudioCode.ps1
    Runs the detection check and exits 0 when Visual Studio Code is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetVisualStudioCode.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetVisualStudioCode.ps1
