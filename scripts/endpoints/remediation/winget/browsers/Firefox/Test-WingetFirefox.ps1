<#
.SYNOPSIS
    Detects pending updates for Mozilla Firefox (winget id Mozilla.Firefox) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Firefox update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - Firefox is up to date, not installed, or the check could not run (network down,
...
.EXAMPLE
    PS C:\> .\Test-WingetFirefox.ps1
    Runs the detection check and exits 0 when Firefox is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetFirefox.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetFirefox.ps1
