<#
.SYNOPSIS
    Updates Mozilla Firefox (winget id Mozilla.Firefox) silently for Intune Proactive Remediations.

.DESCRIPTION
    Remediation half of the Firefox update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the Firefox process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetFirefox.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetFirefox.ps1
    Updates Mozilla Firefox silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetFirefox.ps1 -WhatIf
