<#
.SYNOPSIS
    Detects pending updates for Google Chrome (winget id Google.Chrome) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the Chrome update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    Exit codes:
    - 0: compliant - Chrome is up to date, not installed, or the check could not run (network down,
...
.EXAMPLE
    PS C:\> .\Test-WingetGoogleChrome.ps1
    Runs the detection check and exits 0 when Chrome is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\...\Test-WingetGoogleChrome.ps1'
    Runs the same check from Intune Management Extension under the SYSTEM context.

.NOTES
    File Name: Test-WingetGoogleChrome.ps1
