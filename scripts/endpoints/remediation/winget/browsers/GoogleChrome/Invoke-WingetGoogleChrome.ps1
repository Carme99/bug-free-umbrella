<#
.SYNOPSIS
    Updates Google Chrome (winget id Google.Chrome) silently for Intune Proactive Remediations.

.DESCRIPTION
    Remediation half of the Chrome update pair. Prefers the Microsoft.WinGet.Client PowerShell
    ...
    If the Chrome process is running, the update is deferred (exit 1) so Intune retries later.
    Exit codes:
...
.NOTES
    File Name: Invoke-WingetGoogleChrome.ps1
...
.EXAMPLE
    PS C:\> .\Invoke-WingetGoogleChrome.ps1
    Updates Google Chrome silently; exits 0 on success or when already up to date.

.EXAMPLE
    PS C:\> .\Invoke-WingetGoogleChrome.ps1 -WhatIf
