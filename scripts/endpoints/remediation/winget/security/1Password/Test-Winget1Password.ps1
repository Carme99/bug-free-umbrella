<#
.SYNOPSIS
    Detects pending 1Password updates (winget id AgileBits.1Password) for Intune Proactive Remediations.

.DESCRIPTION
    Detection half of the 1Password update pair. Checks whether an update is available and
    ...
    Exit codes:
    - 0: compliant - 1Password is up to date, not installed, the device is offline, or the check failed.
    - 1: non-compliant - an application update is available.

.NOTES
    File Name: Test-Winget1Password.ps1
...
.EXAMPLE
    PS C:\> .\Test-Winget1Password.ps1
    Runs the check; exits 0 when 1Password is up to date or absent, 1 when an update is available.

.EXAMPLE
    PS C:\> & 'C:\Program Files\IntuneScripts\Test-Winget1Password.ps1'
...
$ConnectivityHost = 'www.microsoft.com'  # Host used for the connectivity check
    ...
    try {
        ...
        if ($packageInfo -match '\bVersion\s+Available\b') {
            ...
            if ($packageInfo -match '\d+(\.\d+)+') {
                ...
                Write-Host "[!] Could not parse version info for $name; treating as compliant." -ForegroundColor Yellow
                return 0
            }
        }
    }
        ...
        Write-Host "[-] Failed to check $name for updates: $($_.Exception.Message)" -ForegroundColor Red
        return 0
    }
    finally {
