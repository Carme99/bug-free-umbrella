<#
.SYNOPSIS
    Audit Exchange Online distribution lists, Microsoft 365 Groups, and mail-enabled security groups.
...
.DESCRIPTION
    ...
    This is a read-only audit: it never modifies Exchange Online objects; it only reads group data
    and writes a report file into the folder given by -OutputPath.
...
.EXAMPLE
    ...
    PS C:\> .\Get-DistributionListAudit.ps1
    Audits all distribution lists with default settings and writes an HTML report.

.EXAMPLE
    PS C:\> .\Get-DistributionListAudit.ps1 -IncludeMicrosoft365Groups `
        -IncludeSecurityGroups -CheckForOrphaned -MinimumMembers 3
    Audits distribution lists plus Microsoft 365 Groups and security groups, flagging orphaned
    groups and groups below the member threshold.

.NOTES
    File Name  : Get-DistributionListAudit.ps1
...
function Main {
    try {
        ...
        Write-Host "`n[*] Retrieving distribution lists..." -ForegroundColor Cyan
        $distributionLists = Get-DistributionGroup -ResultSize Unlimited -ErrorAction Stop
        ...
        foreach ($dl in $distributionLists) {
            ...
            $defaultDomain = (Get-AcceptedDomain | Where-Object { $_.Default }).DomainName.Domain
            $externalMembers = $members |
                Where-Object { $_.PrimarySmtpAddress -notlike "*@$defaultDomain" }
            if ($externalMembers) {
        ...
        Write-Host "[+] Found $($distributionLists.Count) distribution lists" -ForegroundColor Green

        # Get Microsoft 365 Groups
        if ($IncludeMicrosoft365Groups) {
            Write-Host "`n[*] Retrieving Microsoft 365 Groups..." -ForegroundColor Cyan
            ...
            foreach ($group in $m365Groups) {
                $members = Get-UnifiedGroupLinks -Identity $group.Identity `
                    -LinkType Members -ErrorAction SilentlyContinue
                $owners = Get-UnifiedGroupLinks -Identity $group.Identity `
                    -LinkType Owners -ErrorAction SilentlyContinue
        ...
        if ($IncludeSecurityGroups) {
            Write-Host "`n[*] Retrieving mail-enabled security groups..." -ForegroundColor Cyan
            ...
            foreach ($sg in $securityGroups) {
                ...
                $memberCount = ($members | Measure-Object).Count
                $sgNotes = if ($memberCount -lt $MinimumMembers) { "Below minimum member threshold" } else { "" }
            ...
            Write-Host "[+] Found $($securityGroups.Count) mail-enabled security groups" -ForegroundColor Green
        }
    }
    catch {
    ...
    switch ($OutputFormat) {
        'Console' {
            ...
            $orphanedColor = if ($results.OrphanedGroups.Count -gt 0) { 'Yellow' } else { 'White' }
            $smallColor = if ($results.SmallGroups.Count -gt 0) { 'Yellow' } else { 'White' }
            Write-Host "Orphaned Groups (no owners): $($results.OrphanedGroups.Count)" -ForegroundColor $orphanedColor
            Write-Host ("Small Groups (< $MinimumMembers members): " +
                "$($results.SmallGroups.Count)") -ForegroundColor $smallColor
        ...
        'HTML' {
            ...
            $orphanedValueColor = if ($results.OrphanedGroups.Count -gt 0) { '#d13438' } else { '#107c10' }
            $smallValueColor = if ($results.SmallGroups.Count -gt 0) { '#ff8c00' } else { '#107c10' }
...
<body>
    ...
    <div class="summary">
        ...
        <div class="summary-grid">
            ...
            <div class="summary-item">
                <div class="value" style="color: $orphanedValueColor;">$($results.OrphanedGroups.Count)</div>
                <div class="label">Orphaned</div>
            </div>
            <div class="summary-item">
                <div class="value" style="color: $smallValueColor;">$($results.SmallGroups.Count)</div>
...
"@
    ...
    <div class="footer">
        ...
        Please validate results before making changes.<br>
...
"@
        ...
        'CSV' {
            ...
            $results.DistributionLists | Export-Csv -Path $csvFile -NoTypeInformation -ErrorAction Stop
    ...
    Write-Host "`n[+] Distribution list audit complete!" -ForegroundColor Green
    return 0
}
