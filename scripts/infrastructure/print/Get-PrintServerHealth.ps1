...
.DESCRIPTION
    Checks print server infrastructure including Print Spooler service status, printer status and
    errors, print queue health, stuck/stale print jobs, printer driver issues, spool directory disk
    space, and recent event log errors, then prints a summary with a computed health score.
    Side effects: optionally removes stuck print jobs when -ClearStuckJobs is supplied (guarded by
    -WhatIf/-Confirm); optionally writes HTML/CSV reports under the user's Documents\Reports folder.
...
.EXAMPLE
    PS C:\> .\Get-PrintServerHealth.ps1
    Basic print server health check.

.EXAMPLE
    PS C:\> .\Get-PrintServerHealth.ps1 -ClearStuckJobs -StuckJobThresholdHours 4
    ...
    try {
        ...
        try {
            ...
            foreach ($printer in $printers) {
                ...
                if ($status -eq "Fail") {
                    Write-Host "[-]     $($printer.Name): $($printer.PrinterStatus)" -ForegroundColor Red
                    $issueCount++
                }
                elseif ($status -eq "Warning") {
                    Write-Host "[!]     $($printer.Name): $($printer.PrinterStatus)" -ForegroundColor Yellow
                    $warningCount++
                }
                else {
                    Write-Host "[+]     $($printer.Name): $($printer.PrinterStatus)" -ForegroundColor Green
                }
                ...
                if ($jobs.Count -gt 0) {
                    ...
                    foreach ($job in $jobs) {
                        ...
                        if ($jobAge.TotalHours -gt $StuckJobThresholdHours) {
                            ...
                            $jobAgeHours = [math]::Round($jobAge.TotalHours, 1)
                            Write-Host ("[!]     Stuck job on $($printer.Name): $($job.DocumentName) " +
                            "($jobAgeHours hours old)") -ForegroundColor Yellow
            ...
            $stuckColor = if ($stuckJobs.Count -eq 0) { "Green" } else { "Yellow" }
            Write-Host "Stuck jobs: $($stuckJobs.Count)" -ForegroundColor $stuckColor
            ...
            if ($ClearStuckJobs -and $stuckJobs.Count -gt 0) {
                ...
                foreach ($job in $stuckJobs) {
                    if ($PSCmdlet.ShouldProcess("$($job.DocumentName) on $($job.PrinterName)",
        ...
        try {
            ...
            if ($errorEvents.Count -gt 0) {
                Write-Host "[!] Found $($errorEvents.Count) print service error(s) in last 24 hours" `
                    -ForegroundColor Yellow
                $warningCount++

                foreach ($eventRecord in ($errorEvents | Select-Object -First 5)) {
                    $messagePreview = $eventRecord.Message.Substring(0, [Math]::Min(100, $eventRecord.Message.Length))
                    Write-Host "    - $($eventRecord.TimeCreated): $messagePreview" -ForegroundColor Gray
                }
            }
            else {
        ...
        # Export results
        if ($ExportHTML) {
            $htmlPath = Join-Path $reportDir "PrintServerHealth_$($script:Timestamp).html"
...
<html>
<head>
    <title>Print Server Health - $($script:Timestamp)</title>
    <style>
...
"@
        ...
        if ($ExportCSV) {
            $csvPath = Join-Path $reportDir "PrintServerHealth_$($script:Timestamp).csv"
        ...
        Write-Host "[-] Error during print server health check: $($_.Exception.Message)" -ForegroundColor Red
        return 1
    }
