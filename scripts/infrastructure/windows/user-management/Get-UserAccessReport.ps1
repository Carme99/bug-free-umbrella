...
.EXAMPLE
    .\Get-UserAccessReport.ps1 -Username "jdoe" -OutputPath "C:\Reports"
    Audits specific user's access rights.

.EXAMPLE
    .\Get-UserAccessReport.ps1 -IncludeFileShares -CheckLocalAdmin -ExportToCSV
...
try {
    ...
    Write-Host "\nDomain: $domainName" -ForegroundColor Green

    # Get users to audit
    if ($Username) {
        Write-Host "`nAuditing specific user: $Username" -ForegroundColor Yellow
        $users = @(Get-ADUser -Identity $Username -Properties *)
    }
    else {
        Write-Host "`nRetrieving all enabled users..." -ForegroundColor Yellow
    ...
    foreach ($user in $users) {
        $counter++
        Write-Progress -Activity "Auditing Users" -Status "Processing $($user.SamAccountName) ($counter of $($users.Count))" -PercentComplete (($counter / $users.Count) * 100)
    ...
    Write-Host "Privileged Users: $(($accessReports | Where-Object IsPrivileged).Count)" -ForegroundColor $(if (($accessReports | Where-Object IsPrivileged).Count -gt 0) { 'Yellow' } else { 'Green' })
    ...
    Write-Host "`nTop 10 Users by Group Membership:" -ForegroundColor Cyan
...
"@
    ...
    Write-Error "Error generating access report: $_"
