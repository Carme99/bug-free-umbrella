...
.DESCRIPTION
    ...
    Supports monitoring across multiple repositories and organizations. Authenticates with a
    GitHub token (-GitHubToken or GITHUB_TOKEN) and writes an HTML/CSV/JSON report under -OutputPath.
...
.EXAMPLE
    PS C:\> .\Monitor-GitHubActions.ps1 -Owner 'myorg' -Repository 'myrepo' -GitHubToken $env:GITHUB_TOKEN

.EXAMPLE
    PS C:\> .\Monitor-GitHubActions.ps1 -Owner 'myorg' -Repository '*' -DaysToAnalyze 30 -IncludeRunners -IncludeBilling

.NOTES
    File Name: Monitor-GitHubActions.ps1
...
function Main {
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($OutputPath) -or
            $OutputPath -match '(^|[\\/])\.\.([\\/]|$)' -or
            $OutputPath -match '^(\\\\|//)') {
            Write-Error "Unsafe OutputPath: $OutputPath. Must not contain '..' traversal or be a UNC/remote path."
            return 1
        }
        ...
        $startMessage = "[*] Analyzing GitHub Actions workflows for $Owner/$Repository (Last $DaysToAnalyze days)..."
        ...
        foreach ($repo in $repositories) {
            Write-Host "`n[*] Analyzing repository: $repo" -ForegroundColor Cyan
            ...
            try {
                ...
                foreach ($workflow in $workflowsResponse.workflows) {
                    Write-Host "  [*] Analyzing workflow: $($workflow.name)" -ForegroundColor Cyan

                    # Get workflow runs
                    $runsUrl = "$baseUrl/repos/$Owner/$repo/actions/workflows/$($workflow.id)/runs" +
                        "?per_page=100&created=>=$($startDate.ToString('yyyy-MM-dd'))"
        ...
        if ($IncludeRunners) {
            Write-Host "`n[*] Analyzing self-hosted runners..." -ForegroundColor Cyan
            try {
        ...
        if ($IncludeBilling) {
            Write-Host "`n[*] Retrieving billing information..." -ForegroundColor Cyan
            try {
        ...
        switch ($OutputFormat) {
            'Console' {
                ...
                $healthSummary = "Healthy: $healthyWorkflows | Warning: $warningWorkflows" +
                ...
                foreach ($workflow in $results.Workflows | Sort-Object SuccessRate) {
                    ...
                    $detail = "$($workflow.Repository)/$($workflow.WorkflowName): $($workflow.SuccessRate)% success" +
        ...
        <head>
            ...
            <style>
                ...
                .summary-item {
                    background: #f6f8fa; padding: 15px; border-radius: 5px;
                    text-align: center; border: 1px solid #d1d5da;
                }
        ...
        <body>
            ...
            <div class="summary">
                <strong>Owner:</strong> $([System.Net.WebUtility]::HtmlEncode("$Owner")) |
                <strong>Repository:</strong> $([System.Net.WebUtility]::HtmlEncode("$Repository")) |
                <strong>Period:</strong> Last $DaysToAnalyze days<br>
...
"@
        ...
        Write-Host "`n[+] GitHub Actions workflow analysis complete!" -ForegroundColor Green
