...
.DESCRIPTION
    ...
    Authenticates with a GitLab personal access token (-PrivateToken or GITLAB_TOKEN) against the
    v4 API and writes an HTML/CSV/JSON report under -OutputPath.
...
.EXAMPLE
    PS C:\> .\Monitor-GitLabCI.ps1 -GitLabUrl 'https://gitlab.com' -ProjectId 'mygroup/myproject'

.EXAMPLE
    PS C:\> .\Monitor-GitLabCI.ps1 -GitLabUrl 'https://gitlab.company.com' -ProjectId '*' -DaysToAnalyze 30

.NOTES
    File Name: Monitor-GitLabCI.ps1
...
function Main {
    try {
        ...
        $startMessage = "[*] Analyzing GitLab CI pipelines for $ProjectId (Last $DaysToAnalyze days)..."
        ...
        try {
            if ($ProjectId -eq '*') {
                ...
                $project = Invoke-RestMethod -Uri $projectUrl -Headers $headers -Method Get -ErrorAction Stop
        ...
        foreach ($project in $projects) {
            Write-Host "`n[*] Analyzing project: $($project.path_with_namespace)" -ForegroundColor Cyan
            ...
            try {
                ...
                if ($pipelinesResponse.Count -eq 0) {
                    Write-Host "  [*] No recent pipelines found" -ForegroundColor Cyan
                    continue
                }
            ...
            if ($IncludeDeployments) {
                try {
                    $deploymentsUrl = "$apiUrl/projects/$($project.id)/deployments" +
                        "?updated_after=$startDate&per_page=100"
                    ...
                    Write-Warning ("Failed to retrieve deployments for $($project.path_with_namespace): " +
                        "$($_.Exception.Message)")
                }
            }
        }
        ...
        if ($IncludeRunners) {
            Write-Host "`n[*] Analyzing GitLab Runners..." -ForegroundColor Cyan
            try {
                $runnersUrl = "$apiUrl/runners/all?per_page=100"
                $runnersResponse = Invoke-RestMethod -Uri $runnersUrl -Headers $headers -Method Get -ErrorAction Stop
        ...
        switch ($OutputFormat) {
            'Console' {
                ...
                foreach ($pipeline in $results.Pipelines | Sort-Object SuccessRate) {
                    ...
                    $pipelineLine = "$($pipeline.Project)@$($pipeline.Ref): $($pipeline.SuccessRate)% success" +
                        " ($($pipeline.TotalRuns) runs)"
                    Write-Host $pipelineLine -ForegroundColor $color
                }
                ...
                if ($IncludeRunners -and $results.Runners.Count -gt 0) {
                    ...
                    foreach ($runner in $results.Runners) {
                        ...
                        $runnerLine = "$($runner.Description): $status" +
                            " - $($runner.Platform)/$($runner.Architecture)"
                        Write-Host $runnerLine -ForegroundColor White
                    }
                }
            }
...
"@
        ...
        Write-Host "`n[+] GitLab CI pipeline analysis complete!" -ForegroundColor Green
