...
.EXAMPLE
    .\Analyze-BuildPerformance.ps1 -Platform "AzureDevOps" -DataSource ".\builds.json" -IdentifyRegressions

.EXAMPLE
    .\Analyze-BuildPerformance.ps1 -Platform "GitHub" -DataSource ".\builds.json" -DaysToAnalyze 60 -RegressionThreshold 30
...
Write-Host "Analyzing $($filteredBuilds.Count) builds..." -ForegroundColor Yellow
...
$weeklyTrends = $filteredBuilds | Group-Object {
    ...
    @{
        ...
        MedianDuration = [math]::Round(($weekBuilds.DurationMinutes | Sort-Object)[[math]::Floor($weekBuilds.Count / 2)], 2)
        SuccessRate = [math]::Round((($weekBuilds | Where-Object { $_.Result -eq 'Success' }).Count / $weekBuilds.Count) * 100, 2)
    }
}
...
if ($filteredBuilds[0].Stages) {
    Write-Host "`nAnalyzing build stages..." -ForegroundColor Cyan
    ...
    foreach ($stageName in $allStages.Keys) {
        ...
        $analysis.StageAnalysis += @{
            ...
            MedianDuration = [math]::Round(($stageDurations | Sort-Object)[[math]::Floor($stageDurations.Count / 2)], 2)
...
if ($IdentifyRegressions) {
    Write-Host "`nDetecting performance regressions..." -ForegroundColor Cyan
    ...
    if ($baselineBuilds.Count -gt 5) {
        ...
        foreach ($build in $recentBuilds) {
            if ($build.DurationMinutes -gt $regressionThresholdValue) {
                $increasePercent = [math]::Round((($build.DurationMinutes - $baselineAvg) / $baselineAvg) * 100, 2)
        ...
        Write-Host "Found $($analysis.Regressions.Count) potential regressions (>$RegressionThreshold% slower than baseline)" -ForegroundColor Yellow
    }
}
...
switch ($OutputFormat) {
    'Console' {
        ...
        Write-Host "Average Duration: $avgDuration min | Median: $medianDuration min | 95th: $p95Duration min" -ForegroundColor White
        ...
        if ($analysis.StageAnalysis.Count -gt 0) {
            ...
            $slowestStages = $analysis.StageAnalysis | Sort-Object AvgDuration -Descending | Select-Object -First 5
            foreach ($stage in $slowestStages) {
                Write-Host "$($stage.StageName): Avg $($stage.AvgDuration) min (Max: $($stage.MaxDuration) min)" -ForegroundColor White
            }
        }
        ...
        if ($analysis.Regressions.Count -gt 0) {
            ...
            foreach ($regression in ($analysis.Regressions | Select-Object -First 5)) {
                Write-Host "Build $($regression.BuildId): $($regression.DurationMinutes) min (+$($regression.IncreasePercent)% vs baseline)" -ForegroundColor Red
            }
        }
    }
...
Write-Host "`nBuild performance analysis complete!" -ForegroundColor Green
