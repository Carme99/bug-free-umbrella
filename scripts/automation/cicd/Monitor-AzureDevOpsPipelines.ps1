...
.DESCRIPTION
    ...
    Supports both Build and Release pipelines across multiple projects. Reads the PAT from the
    AZURE_DEVOPS_PAT environment variable and writes an HTML/CSV/JSON report under -OutputPath.
...
.EXAMPLE
    PS C:\> $env:AZURE_DEVOPS_PAT = 'your-pat-here'
    PS C:\> .\Monitor-AzureDevOpsPipelines.ps1 -Organization 'mycompany' -Project 'MyProject'

    Supplies the PAT via the environment variable (never as a CLI argument) and analyzes one project.

.EXAMPLE
    PS C:\> .\Monitor-AzureDevOpsPipelines.ps1 -Organization 'mycompany' -Project '*' -DaysToAnalyze 30 -IncludeReleases

.NOTES
    File Name: Monitor-AzureDevOpsPipelines.ps1
...
# Map a success rate percentage to a health status label.
function Get-HealthStatus {
    param([int]$SuccessRate)
    if ($SuccessRate -ge 90) { 'Healthy' }
    elseif ($SuccessRate -ge 70) { 'Warning' }
    else { 'Critical' }
}
...
function Main {
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($PersonalAccessToken)) {
            Write-Error "Personal Access Token is required. Set AZURE_DEVOPS_PAT before running, e.g.:
        ...
        $isUncPath = $rawOutputPath.StartsWith('\\', [System.StringComparison]::Ordinal) -or
            $rawOutputPath.StartsWith('//', [System.StringComparison]::Ordinal)
        if ($isUncPath) {
        ...
        $startMessage = "[*] Analyzing Azure DevOps pipelines for $Organization/$Project (Last $DaysToAnalyze days)..."
        ...
        try {
            if ($Project -eq '*') {
                ...
                $projectsResponse = Invoke-RestMethod -Uri $projectsUrl -Headers $headers -Method Get -ErrorAction Stop
        ...
        foreach ($proj in $projects) {
            ...
            Write-Host "`n[*] Analyzing project: $projectName" -ForegroundColor Cyan

            # Get pipeline definitions
            try {
                $pipelinesUrl = "$baseUrl/$projectName/_apis/pipelines?api-version=7.0"
                $pipelinesResponse = Invoke-RestMethod -Uri $pipelinesUrl -Headers $headers `
                    -Method Get -ErrorAction Stop

                foreach ($pipeline in $pipelinesResponse.value) {
                    Write-Host "  [*] Analyzing pipeline: $($pipeline.name)" -ForegroundColor Cyan
                    ...
                    $successfulRuns = ($recentRuns | Where-Object {
                        $_.state -eq 'completed' -and $_.result -eq 'succeeded'
                    }).Count
                    ...
                    $successRate = if ($totalRuns -gt 0) {
                        [math]::Round(($successfulRuns / $totalRuns) * 100, 2)
                    }
                    else { 0 }
                    ...
                    $results.Pipelines += @{
                        ...
                        Status = (Get-HealthStatus $successRate)
                    }
                }
            }
            catch {
            ...
            if ($IncludeAgentPools) {
                try {
                    ...
                    $poolsResponse = Invoke-RestMethod -Uri $poolsUrl -Headers $headers -Method Get -ErrorAction Stop

                    foreach ($pool in $poolsResponse.value) {
                        $agentsUrl = "$baseUrl/_apis/distributedtask/pools/$($pool.id)/agents?api-version=7.0"
                        $agentsResponse = Invoke-RestMethod -Uri $agentsUrl -Headers $headers `
                            -Method Get -ErrorAction Stop
                ...
                if ($onlineAgents -eq 0) { $healthStatus = 'Critical' }
                elseif ($offlineAgents -gt 0) { $healthStatus = 'Warning' }
            ...
            if ($IncludeReleases) {
                try {
                    $releaseUrl = "https://vsrm.dev.azure.com/$Organization/$projectName/_apis/release/releases" +
                        "?api-version=7.0&minCreatedTime=$startDate"
                    $releasesResponse = Invoke-RestMethod -Uri $releaseUrl -Headers $headers `
                        -Method Get -ErrorAction Stop
        ...
        switch ($OutputFormat) {
            'Console' {
                ...
                $healthSummary = "Healthy: $healthyPipelines | Warning: $warningPipelines" +
                    " | Critical: $criticalPipelines"
                Write-Host $healthSummary -ForegroundColor White
                ...
                foreach ($pipeline in $results.Pipelines | Sort-Object SuccessRate) {
                    ...
                    $detail = "$($pipeline.Project)/$($pipeline.PipelineName): $($pipeline.SuccessRate)% success" +
                        " ($($pipeline.TotalRuns) runs, avg $($pipeline.AverageDurationMinutes) min)"
                ...
                if ($IncludeAgentPools -and $results.Agents.Count -gt 0) {
                    ...
                    foreach ($agent in $results.Agents) {
                        $agentLine = "$($agent.PoolName): $($agent.OnlineAgents)/$($agent.TotalAgents) agents online"
                        Write-Host $agentLine -ForegroundColor White
                    }
                }
            }
        ...
        <body>
            ...
            <div class="summary">
                <strong>Organization:</strong> $(ConvertTo-HtmlEncoded $Organization) |
                <strong>Analysis Period:</strong> Last $(ConvertTo-HtmlEncoded $DaysToAnalyze) days<br>
...
"@
        ...
        Write-Host "`n[+] Azure DevOps pipeline analysis complete!" -ForegroundColor Green
