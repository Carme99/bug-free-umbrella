...
[void]$sb.AppendLine('# Not published to the PowerShell Gallery. Build and import it from a clone:')
[void]$sb.AppendLine('git clone https://github.com/Carme99/bug-free-umbrella.git')
[void]$sb.AppendLine('Import-Module ./bug-free-umbrella/src/BugFreeUmbrella')
...
[void]$sb.AppendLine('')
[void]$sb.AppendLine('```powershell')
[void]$sb.AppendLine('# Publication has never run: the tag-triggered release-module.yml workflow SKIPS its')
[void]$sb.AppendLine('# publish step unless PSGALLERY_API_KEY is set, and uploads the module artifact instead.')
[void]$sb.AppendLine('# One-time: set the PSGallery API key (never commit)')
