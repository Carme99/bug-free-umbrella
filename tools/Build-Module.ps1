...
.DESCRIPTION
    Reads the catalog (one entry per catalogued script), derives Approved-Verb function names via leaf-dir
...
if ($scripts.Count -ne $catalog.totalScripts) {
    Write-Warning "Catalog scripts array ($($scripts.Count)) does not match totalScripts ($($catalog.totalScripts))."
}
...
<#
.SYNOPSIS
    BugFreeUmbrella module loader — one proxy function per catalogued script, generated by tools/Build-Module.ps1.
...
@{
    ...
    Description        = '$($catalog.totalScripts) catalogued PowerShell scripts ($($catalog.onDiskScripts) on disk) as an importable module'
