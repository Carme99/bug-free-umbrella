...
.DESCRIPTION
    ...
    Returns exit code 0 if update is needed (triggers remediation).
    Returns exit code 1 if app is up-to-date (no remediation needed).
...
try {
    ...
    if (-not `$wingetPath) {
        ...
        if (`$wingetExe) {
            ...
            exit 1  # Compliant - can't remediate without winget
        }
    }
    ...
    if (`$LASTEXITCODE -ne 0) {
        ...
        exit 1  # Compliant - package not installed or not found
    }
    ...
    if (`$upgradeAvailable) {
        ...
        exit 0  # Non-compliant - trigger remediation
    }
    else {
    ...
    exit 1  # Compliant - error during detection
}
"@
