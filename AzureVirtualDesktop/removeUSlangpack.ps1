...
.EXAMPLE
    .\Remove-USLanguagePack-Win11.ps1
    Removes US language pack with confirmation prompts.

.EXAMPLE
    .\Remove-USLanguagePack-Win11.ps1 -Force
    Forces removal without prompts.

.EXAMPLE
    .\Remove-USLanguagePack-Win11.ps1 -KeepKeyboard
    ...
    if (-not $Force) {
        ...
        if ($addNow -eq "Y" -or $addNow -eq "y") {
            ...
            try {
                ...
                Write-Log "Please sign out and back in, then run this script again to complete removal." "WARNING"
                exit 0
            }
            catch {
...
if ($installedLanguages[0].LanguageTag -ne "en-GB") {
    ...
    if (-not $Force) {
        ...
        if ($setDefault -eq "Y" -or $setDefault -eq "y") {
            ...
            try {
                $langList = Get-WinUserLanguageList
                $enGBLang = $langList | Where-Object { $_.LanguageTag -eq "en-GB" }
                $otherLangs = $langList | Where-Object { $_.LanguageTag -ne "en-GB" -and $_.LanguageTag -ne "en-US" }
                
                $newList = New-WinUserLanguageList -Language "en-GB"
...
try {
    $usCaps = Get-WindowsCapability -Online | Where-Object {
...
try {
    ...
    if (Test-Path $userLangPath) {
        ...
        if ($languages -and $languages.Languages -contains "en-US") {
            ...
            Set-ItemProperty -Path $userLangPath -Name "Languages" -Value $newLangs -ErrorAction SilentlyContinue
